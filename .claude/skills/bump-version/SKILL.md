...
description: >
  ...
  自动同步 8 处版本号 + 裁剪 changelog 到 5 条 + 强制文档同步提醒。
---
...
6. 开头 `> **当前版本：vX.Y.Z**`
7. 版本历史追加新条目
8. 代码规模注释中的版本号（⚠️ 仅改版本号字符串，不更新行数数据）
...
**不可跳过。** 必须告知用户以下也需要同步（handoff 流程会自动处理，但单独发版时需要手动）：

```
⚠️ 版本号已同步 8 处。以下文档也需要更新：
  □ CLAUDE.md — 标题版本号 + 本次会话变更节 + 文件结构行数
  □ CODEBUDDY.md — 标题版本号 + 已知问题表 + 文件行数表
  □ .trae/rules/project_rules.md — 标题版本号 + 关键文件行数表 + 已知问题
  □ README.md — 代码规模表（行数实测数据）
```
