...
'use strict';
  ...
  function create(spawnCtx) {
    ...
    // 导弹系统
    var _missileAmmoL = 4, _missileAmmoR = 4;
    var _nextMissileSide = 'L';
    var _reloadTimerL = 0, _reloadTimerR = 0;
    ...
    function _findBestLockTarget(camera, mx, my) {
      ...
      var bx = mx - LOCK_BOX_W/2, by = my - LOCK_BOX_H/2;
    ...
    function _spawnPlayerMissile(target) {
      ...
      // 扣除弹药
      if (side === 'L') _missileAmmoL--; else _missileAmmoR--;
      ...
      if (!pv) return;
      var pivWorld = new THREE.Vector3(); pv.getWorldPosition(pivWorld);
      var pivFwd = new THREE.Vector3(-1, 0.6, 0); pv.localToWorld(pivFwd);
      var launchDir = pivFwd.clone().sub(pivWorld).normalize();
      var muzzlePos = pivWorld.clone().addScaledVector(launchDir, 0.5);
      // 导弹模型 (复用 enemy 导弹样式)
      var grp = new THREE.Group();
      var bm = new THREE.MeshStandardMaterial({ color: '#889999', roughness: 0.3, metalness: 0.7, emissive: '#111111' });
      var body = new THREE.Mesh(new THREE.CylinderGeometry(0.06,0.06,0.4,6), bm); body.rotation.x = Math.PI/2; grp.add(body);
      var tip = new THREE.Mesh(new THREE.ConeGeometry(0.06,0.12,6), bm); tip.rotation.x = -Math.PI/2; tip.position.z = 0.26; grp.add(tip);
      var flame = new THREE.Mesh(new THREE.CylinderGeometry(0.04,0.07,0.3,6), new THREE.MeshBasicMaterial({color:'#ff6600'}));
      flame.rotation.x = Math.PI/2; flame.position.z = -0.35; grp.add(flame);
      var fl = new THREE.PointLight('#ff4400',3,2,2); fl.position.z = -0.4; grp.add(fl);
      grp.position.copy(muzzlePos); _scene.add(grp);
      _playerMissiles.push({
        mesh: grp, pos: muzzlePos.clone(), dir: launchDir.clone(),
        speed: 20, damage: 25, blastRadius: 1.5,
        age: 0, tracking: false, maxTurnRate: 1.2,
        target: target, lastTargetPos: target.position.clone()
      });
    ...
    function _updatePlayerMissiles(dt) {
      for (var mi = _playerMissiles.length-1; mi >= 0; mi--) {
        var m = _playerMissiles[mi]; m.age += dt;
        if (m.age > 0.25) m.tracking = true;
	        ...
	        if (m.tracking && m.lastTargetPos) {
	          ...
	          if (angle > 0.01) {
	            ...
	            if (rotAxis.length() > 0.001) { m.dir.applyQuaternion(new THREE.Quaternion().setFromAxisAngle(rotAxis, turnAmount)).normalize(); }
	            else { m.dir.copy(toTarget); }
	          }
	        }
	        m.pos.addScaledVector(m.dir, m.speed * dt); m.mesh.position.copy(m.pos);
	        var mq = new THREE.Quaternion(); mq.setFromUnitVectors(new THREE.Vector3(0,0,1), m.dir); m.mesh.setRotationFromQuaternion(mq);
	          ...
	          if (m.pos.distanceTo(m.target.position) < m.blastRadius) {
	            ...
	            if (window.EnemyAI && window.EnemyAI.onEnemyDamaged) {
	              ...
	              if (killed) {
	                ...
	                var ep = m.target.position.clone(); ep.y += 0.5;
          ...
          var el = new THREE.PointLight('#ff8822',15,m.blastRadius*3,2); el.position.copy(m.pos); el.position.y+=0.5; _scene.add(el);
          _playerMuzzleLights.push({light:el,life:0.3});
          var es = new THREE.Mesh(new THREE.SphereGeometry(m.blastRadius*0.5,8,6), new THREE.MeshBasicMaterial({color:'#ff6622',transparent:true,opacity:0.8}));
          es.position.copy(m.pos); es.position.y+=1.0; _scene.add(es);
          _playerMissileExplosions.push({mesh:es,life:0.5});
          _scene.remove(m.mesh); m.mesh.traverse(function(c){if(c.geometry)c.geometry.dispose();if(c.material)c.material.dispose();});
          _playerMissiles.splice(mi,1);
        }
      }
      // 爆炸球衰减
      for (var ei=_playerMissileExplosions.length-1;ei>=0;ei--) {
        var he=_playerMissileExplosions[ei]; he.life-=dt;
        he.mesh.scale.multiplyScalar(1+dt*5); he.mesh.material.opacity=Math.max(0,he.life/0.5);
        if (he.life<=0){_scene.remove(he.mesh);he.mesh.geometry.dispose();he.mesh.material.dispose();_playerMissileExplosions.splice(ei,1);}
      }
    }
    ...
    function _showAlert(text) {
      if (!_alertSprite) {
        ...
        _alertSprite = new THREE.Sprite(new THREE.SpriteMaterial({
      ...
      ctx.fillStyle = '#ff3333'; ctx.font = 'bold 28px sans-serif'; ctx.textAlign = 'center';
    ...
    function _create3DReloadBars() {
      ...
      var bgMat = new THREE.MeshBasicMaterial({ color: '#333', side: THREE.DoubleSide, depthTest: false, depthWrite: false, transparent: true, opacity: 0.8 });
      // 左装填条
      _reloadBarL = new THREE.Group();
      var lbBg = new THREE.Mesh(bgGeo, bgMat); lbBg.renderOrder = 999; _reloadBarL.add(lbBg);
      var lFillGeo = new THREE.PlaneGeometry(0.10, 0.49);
      var lFill = new THREE.Mesh(lFillGeo, new THREE.MeshBasicMaterial({ color: '#44ff44', side: THREE.DoubleSide, depthTest: false, depthWrite: false, transparent: true, opacity: 1 }));
      lFill.renderOrder = 1000; lFill.position.z = 0.001; _reloadBarL.add(lFill);
      _reloadBarL.userData = { fill: lFill };
      _reloadBarL.visible = false; _scene.add(_reloadBarL);
      // 右装填条
      _reloadBarR = new THREE.Group();
      var rbBg = new THREE.Mesh(bgGeo, bgMat); rbBg.renderOrder = 999; _reloadBarR.add(rbBg);
      var rFill = new THREE.Mesh(lFillGeo.clone(), new THREE.MeshBasicMaterial({ color: '#44ff44', side: THREE.DoubleSide, depthTest: false, depthWrite: false, transparent: true, opacity: 1 }));
      rFill.renderOrder = 1000; rFill.position.z = 0.001; _reloadBarR.add(rFill);
      _reloadBarR.userData = { fill: rFill };
      _reloadBarR.visible = false; _scene.add(_reloadBarR);
    }
    ...
    function _spawnPlayerGatlingBullet(side) {
      ...
      for (var fi = 0; fi < 3 + Math.floor(Math.random() * 3); fi++) {
        ...
        var fMat = new THREE.MeshBasicMaterial({ color: (fi < 2) ? '#ffcc44' : '#ff8800', transparent: true, opacity: 1 });
    ...
    var ctrl = {
      ...
      onSpawn: function (ctx) {
        ...
        // 收集枪管材质 (红热发光, 左右独立)
        _barrelMatsL = []; _barrelMatsR = [];
        for (var bi = 1; bi <= 4; bi++) {
          var bgL = _root.getObjectByName('左加特林枪管' + bi);
          var bgR = _root.getObjectByName('右加特林枪管' + bi);
          if (bgL && bgL.children[0] && bgL.children[0].material) _barrelMatsL.push(bgL.children[0].material);
          if (bgR && bgR.children[0] && bgR.children[0].material) _barrelMatsR.push(bgR.children[0].material);
        }
        ...
        _ai = {
          ...
          // 每侧独立加特林状态
          _leftSpinUp: 0, _rightSpinUp: 0,
          _leftHeat: 0, _rightHeat: 0,
          _leftOverheated: false, _rightOverheated: false,
          _leftFireTimer: 0, _rightFireTimer: 0,
        };
        ...
        _ohCanvas.width = 256; _ohCanvas.height = 64;
        ...
        _create3DReloadBars();
        _missileAmmoL = 4; _missileAmmoR = 4;
        _reloadTimerL = 0; _reloadTimerR = 0;
        _nextMissileSide = 'L';
        _lockState = 'idle'; _lockTarget = null; _lockProgress = 0;
      },
      ...
      update: function (dt, input) {
        ...
        [{ side: 'right', key: '_right', fireField: 'fireLeft', clusterIdx: 1 },
         { side: 'left', key: '_left', fireField: 'fireRight', clusterIdx: 0 }
          ...
          var fireHeld = !!(input[cfg.fireField] && !frozen);
          ...
          if (fireHeld && overheated) {
            if (isLeftBtn && !_jamLockL) { _jamLockL = true; if (typeof playGatlingJamSound === 'function') playGatlingJamSound(); }
            if (!isLeftBtn && !_jamLockR) { _jamLockR = true; if (typeof playGatlingJamSound === 'function') playGatlingJamSound(); }
          }
        ...
        for (var bi = _playerBullets.length - 1; bi >= 0; bi--) {
          ...
          // 1. 障碍物 — 用 Raycaster 横扫 (与瞄准线 _castRay 完全一致的检测方式)
          // 障碍物Mesh经PCM input传入, 避开 let 跨脚本访问问题
          var obsMeshes = input.obstacleMeshes || window.obstacleMeshes || [];
          if (!hitPoint && obsMeshes.length > 0) {
            _rc.set(oldPos, stepDir);
            _rc.far = stepDist + 0.3;
            var ohits = _rc.intersectObjects(obsMeshes, true);
            if (ohits.length > 0) {
              hitPoint = ohits[0].point.clone();
              hitType = 'obstacle';
            }
          }
          ...
          if (!hitPoint) {
            ...
            if (pb.pos.y < gh) {
              ...
              var frac = Math.max(0, Math.min(1, (oldPos.y - oldGH) / ((oldPos.y - oldGH) - (pb.pos.y - gh) + 1e-8)));
        ...
        // ── 4.13 导弹锁定状态机 ──
        var bothReloading = (_missileAmmoL <= 0 && _reloadTimerL > 0) || (_reloadTimerL <= 0 && _missileAmmoL < 4);
        // 简化: 双巢都无弹可用
        var noAmmo = (_missileAmmoL <= 0 && _missileAmmoR <= 0);

        if (!frozen) {
          // 尝试找锁定目标 (每帧更新, 用于锁定框显示)
          var mx = (input.mouseX !== undefined) ? input.mouseX : (typeof mouseX !== 'undefined' ? mouseX : window.innerWidth/2);
          var my = (input.mouseY !== undefined) ? input.mouseY : (typeof mouseY !== 'undefined' ? mouseY : window.innerHeight/2);
          ...
          // 目标距离检查 (对于已有锁定目标)
          var targetInRange = _lockTarget && _lockTarget.position && _root.position.distanceTo(_lockTarget.position) <= LOCK_MAX_RANGE;

          if (_lockState === 'idle') {
            var cantLock = noAmmo || !bestTarget || (bestTarget && _root.position.distanceTo(bestTarget.position) > LOCK_MAX_RANGE);
            if (input.spaceDown && cantLock) {
              // 空格按住但不能锁定 → 持续显示警报; 按下瞬间播一次失败音
              if (input.spaceJustPressed && typeof playGatlingJamSound === 'function') playGatlingJamSound();
            ...
            if (_lockTarget && _lockTarget.dead) {
              ...
              if (_lockProgress >= 1.0) {
                _lockState = 'locked'; _lockProgress = 1.0;
        ...
        if (_reloadBarL && _reloadBarR) {
          var bx = _root.position.x, bz = _root.position.z;
          var by = _gh(bx, bz) + 1.6;  // 地面高度+偏移(六足比坦克高)
          if (input.camera) {
            var cam = input.camera;
            var toCamX = cam.position.x - bx, toCamZ = cam.position.z - bz;
            var tl = Math.sqrt(toCamX*toCamX + toCamZ*toCamZ) || 1;
            var cfx = toCamX/tl, cfz = toCamZ/tl;
            _reloadBarL.position.set(bx + cfz*1.0, by, bz - cfx*1.0); _reloadBarL.lookAt(cam.position);
            _reloadBarR.position.set(bx - cfz*1.0, by, bz + cfx*1.0); _reloadBarR.lookAt(cam.position);
            var lpct = _reloadTimerL > 0 ? (RELOAD_TIME - _reloadTimerL)/RELOAD_TIME : _missileAmmoL/4;
            var rpct = _reloadTimerR > 0 ? (RELOAD_TIME - _reloadTimerR)/RELOAD_TIME : _missileAmmoR/4;
      ...
      dispose: function () {
        ...
        for (var i = _playerBullets.length - 1; i >= 0; i--) {
          ...
          if (b.mesh) { _scene.remove(b.mesh); b.mesh.geometry.dispose(); b.mesh.material.dispose(); }
        }
        _playerBullets = [];
        // 清枪口闪光
        ...
        _playerMuzzleLights = [];
        for (var k = _playerMuzzleFlashes.length - 1; k >= 0; k--) { _scene.remove(_playerMuzzleFlashes[k].mesh); _playerMuzzleFlashes[k].mesh.geometry.dispose(); _playerMuzzleFlashes[k].mesh.material.dispose(); }
        _playerMuzzleFlashes = [];
        // 清过热警报 Sprite
        if (_overheatSprite) {
          if (_overheatSprite.material && _overheatSprite.material.map) _overheatSprite.material.map.dispose();
        ...
        for (var mi = _playerMissiles.length-1; mi>=0; mi--) {
          ...
          _scene.remove(pm.mesh); pm.mesh.traverse(function(c){if(c.geometry)c.geometry.dispose();if(c.material)c.material.dispose();});
        }
        _playerMissiles = [];
        for (var ei = _playerMissileExplosions.length-1; ei>=0; ei--) {
          var ex = _playerMissileExplosions[ei];
          _scene.remove(ex.mesh); ex.mesh.geometry.dispose(); ex.mesh.material.dispose();
        }
        _playerMissileExplosions = [];
        if (_lockCircle) { _scene.remove(_lockCircle); _lockCircle.geometry.dispose(); _lockCircle.material.dispose(); _lockCircle = null; }
        _lockState = 'idle'; _lockTarget = null;
        // 隐藏导弹 UI + 清理 3D 装填条
        var lb = document.getElementById('missile-lock-box'); if (lb) lb.style.display = 'none';
        if (_reloadBarL) { _reloadBarL.traverse(function(c){if(c.geometry)c.geometry.dispose();if(c.material)c.material.dispose();}); _scene.remove(_reloadBarL); _reloadBarL = null; }
        if (_reloadBarR) { _reloadBarR.traverse(function(c){if(c.geometry)c.geometry.dispose();if(c.material)c.material.dispose();}); _scene.remove(_reloadBarR); _reloadBarR = null; }
        if (_alertSprite) { if (_alertSprite.material && _alertSprite.material.map) _alertSprite.material.map.dispose(); if (_alertSprite.material) _alertSprite.material.dispose(); _scene.remove(_alertSprite); _alertSprite = null; }
        if (_obsMesh && _obsMesh.material) { _obsMesh.material.dispose(); _obsMesh = null; }
