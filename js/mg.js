...
function createMGBullet(origin, targetPos, travelDist) {
  ...
  const geo = new THREE.CylinderGeometry(0.06, 0.06, 0.3, 6);
  ...
  return {
    mesh,
    vel: dir.clone().multiplyScalar(MG_BULLET_SPEED),
    origin: origin.clone(),
    life: MG_BULLET_LIFE,
    maxLife: MG_BULLET_LIFE,
...
function updateMGAutoTarget(player, dt) {
  ...
  if (!player.mgLockTarget) {
    ...
    for (const enemy of enemies) {
      ...
      if (d < MG_DETECT_RANGE && d < nearestDist) {
        nearestDist = d;
        nearest = enemy;
      }
    }
  ...
  var muzzleOff =
    player.spec && player.spec.mgMuzzleOffset ? player.spec.mgMuzzleOffset : [0, 0.295, 0.57];
  const mgOrigin = new THREE.Vector3(muzzleOff[0], muzzleOff[1], muzzleOff[2]);
  if (player.mgGroup) {
  ...
  if (mgHeat >= MG_OVERHEAT_TIME) {
    mgOverheated = true;
    mgHeat = MG_OVERHEAT_TIME;
    return;
  }
  ...
  if (player && player.group)
    player.group.traverse((c) => {
      if (c.isMesh || c.isGroup) excludeList.push(c);
    });
  const hitTargets = [...scene.children];
  if (groundPlane && !hitTargets.includes(groundPlane)) hitTargets.push(groundPlane);
  const hits = raycaster
    .intersectObjects(hitTargets, true)
    .filter((h) => !excludeList.includes(h.object));
...
function updateMGBullets(dt) {
  for (let i = mgBullets.length - 1; i >= 0; i--) {
    ...
    if (b.mesh.children.length > 0 && b.mesh.children[0].material)
      b.mesh.children[0].material.opacity = 0.7 * lr;
  }
}
