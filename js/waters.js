...
function _getRiver() {
  return _t('river', null);
}
window._getPond = _getPond;
window._getRiver = _getRiver;
...
function createWaterSurface() {
  ...
  const edgeHs = sAngles.map((ang) =>
    getTerrainHeight(pond.cx + Math.cos(ang) * pond.rx, pond.cz + Math.sin(ang) * pond.rz)
  );
...
function updateWaterAnimation(dt) {
  ...
  if (riverWater && riverWater.geometry && riverWater.geometry.attributes.position) {
    ...
    for (let i = 0; i < rpos.count; i++) {
      const wx = rpos.getX(i),
        wz = -rpos.getY(i); // 局部Y→世界Z（符号翻转）
...
function cleanupWater() {
  ...
  riverColliders = [];
  if (window._riverGrid) {
    window._riverGrid.clear();
    window._riverGrid = null;
  }
}
window.cleanupWater = cleanupWater;
window.isInPond = isInPond;
window.isInRiver = isInRiver;
window.isInAnyRiver = isInAnyRiver;
window.createWaterSurface = createWaterSurface;
window.createRiverWater = createRiverWater;
