...
async function loadMapsFromDirectory() {
  ...
  mapsLoadPromise = (async () => {
    try {
      ...
      if (failed.length > 0)
        console.warn('⚠️ 加载失败:', failed.map((r) => r.reason.message).join(', '));
    } catch (e) {
      console.warn('⚠️ 地图动态加载失败，回退到默认地图:', e.message);
      MAP_CONFIGS['test_map_01a'] = {
        version: '1.0',
        name: '默认地图(离线)',
        size: 200,
        playWidth: 200,
        ...
        terrain: { pond: { cx: 0, cz: 50, rx: 4.5, rz: 6, depth: 5 } },
        terrainTypes: { default: 'grass' },
        waters: { pond: { waterLevel: -1 } },
...
function loadMapConfig(mapId) {
  ...
  if (mapId.startsWith('editor_')) {
    ...
    try {
      ...
      if (bp) {
        ...
        worldHalfW = _eww / 2;
        worldHalfD = _ewd / 2;
        spawnHalfW = playHalfW - 2;
        spawnHalfD = playHalfD - 2;
  ...
  playHalfW = _pw / 2;
  playHalfD = _pd / 2;
  worldHalfW = currentMapData.worldWidth / 2;
  worldHalfD = currentMapData.worldDepth / 2;
  spawnHalfW = playHalfW - 2;
  spawnHalfD = playHalfD - 2;
...
function convertBlueprintToMapConfig(bp) {
  ...
  const spawn = spawnEnt
    ? [spawnEnt.position.x, spawnEnt.position.z, spawnEnt.yaw || 0]
    : [0, 0, 0];
  const enemies = (bp.entities || [])
  ...
  if (validRivers.length > 0) {
    ...
    if (r0.waterLevels && r0.waterLevels.length > 0) {
      ...
      terrainExtra.riverWaterLevel =
        r0.waterLevel != null ? r0.waterLevel : r0.waterLevels[r0.waterLevels.length - 1];
  ...
  const editorBuildings = (bp.entities || [])
    .filter((e) => e.type === 'building')
    .map((e) => ({
      x: e.position.x,
      z: e.position.z,
      type: e.subType || 'bungalow',
      yaw: e.yaw || 0,
    }));
  const editorTrees = (bp.entities || [])
  ...
  let _hm = bp.heightmap,
    _sm = bp.splatMap;
  if (!_hm && bp.heightmapB64) {
    const bin = atob(bp.heightmapB64);
    const b = new Uint8Array(bin.length);
    for (let i = 0; i < bin.length; i++) b[i] = bin.charCodeAt(i);
    _hm = Array.from(new Float32Array(b.buffer));
  }
  if (!_sm && bp.splatMapB64) {
    const bin = atob(bp.splatMapB64);
    _sm = new Uint8Array(bin.length);
  ...
  return {
    ...
    obstacles: {
      ...
      minDist: 6,
      safeRadius: 10,
      spawnRadius: Math.min(_spawnHalfW, _spawnHalfD),
      buildings: editorBuildings,
      editorTrees,
      spawnHalfW: _spawnHalfW,
      spawnHalfD: _spawnHalfD,
    },
...
function smoothNoise(x, z, seed) {
  ...
  const v00 = noise2D(ix, iz, seed),
    v10 = noise2D(ix + 1, iz, seed);
  const v01 = noise2D(ix, iz + 1, seed),
    v11 = noise2D(ix + 1, iz + 1, seed);
  ...
  for (let sy = 0; sy < sizeD; sy++) {
    ...
    for (let sx = 0; sx < sizeW; sx++) {
      ...
      if (t && t.river && tt.riverBank) {
        ...
        const rhw =
          t.river.hwBase + t.river.hwVar * Math.sin(wx / t.river.hwPeriod + t.river.hwPhase);
      ...
      if (t && t.pond && tt.pondEdge) {
        const px = wx - t.pond.cx,
          pz = wz - t.pond.cz;
        const ed = Math.sqrt(
          (px * px) / (t.pond.rx * t.pond.rx) + (pz * pz) / (t.pond.rz * t.pond.rz)
        );
...
function generateCompositeGroundTexture(groundHalf) {
  ...
  const _sw =
    (currentMapData.terrain && (currentMapData.terrain.hmResW || currentMapData.terrain.width)) ||
    256;
  const _sd =
  ...
  const texCanvases = TERRAIN_TYPE_NAMES.map((name, idx) => {
    try {
      ...
      fcx.fillStyle = ['#4a8c3f', '#6b4a2e', '#c4a860', '#b0b0b0', '#4a4a4a', '#a05a3c'][idx];
      fcx.fillRect(0, 0, 256, 256);
      return fc;
    }
  });
  ...
  for (let by = 0; by < outSize; by += blockSize) {
    for (let bx = 0; bx < outSize; bx += blockSize) {
      ...
      const typeIdx =
        sx >= 0 && sx < splatSizeW && sy >= 0 && sy < splatSizeD ? splat[sy * splatSizeW + sx] : 0;
