...
function createSpawnMarker(x, y, z, yaw) {
  ...
  const flag = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xf5494a, side: THREE.DoubleSide, roughness: 0.5 })
  );
  flag.position.set(0.6, 3.5, 0);
  flag.rotation.y = yaw;
  grp.add(flag);
  // 方向箭头
...
function createTreeMarker(x, y, z, stype) {
  ...
  if (stype === 'sphere') {
    const crown = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x4a8b4a, roughness: 0.7 })
    );
    crown.position.y = 3.2;
    grp.add(crown);
  } else if (stype === 'oak') {
    const crown = new THREE.Mesh(
      new THREE.SphereGeometry(1.5, 10, 7),
      new THREE.MeshStandardMaterial({ color: 0x3a7a3a, roughness: 0.7 })
    );
    ...
    const crown = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x2d6b2d, roughness: 0.7 })
    );
    crown.position.y = 3.5;
    grp.add(crown);
  }
...
function createBuildingMarker(x, y, z, stype, yaw) {
  const grp = new THREE.Group();
  // 正面门(+Z面亮色薄盒外突)：marker 是对称低模(盒身+四面坡屋顶)，门是唯一朝向标识，旋转时可见正面
  const doorMat = new THREE.MeshStandardMaterial({ color: 0xffcc00, roughness: 0.5 });
  if (stype === 'villa') {
    ...
    const roof = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x6b4a3a, roughness: 0.7 })
    );
    roof.position.y = 4.6;
    roof.rotation.y = Math.PI / 4;
    grp.add(roof);
    const door = new THREE.Mesh(new THREE.BoxGeometry(0.7, 1.5, 0.1), doorMat);
    door.position.set(0, 0.75, 1.3);
    grp.add(door);
  } else if (stype === 'apartment') {
    const body = new THREE.Mesh(
      new THREE.BoxGeometry(2, 5, 2),
      new THREE.MeshStandardMaterial({ color: 0x707080, roughness: 0.5 })
    );
    body.position.y = 2.5;
    grp.add(body);
    const door = new THREE.Mesh(new THREE.BoxGeometry(0.7, 1.8, 0.1), doorMat);
    door.position.set(0, 0.9, 1.05);
    grp.add(door);
  } else {
    // bungalow
    const body = new THREE.Mesh(
      new THREE.BoxGeometry(2, 3, 2),
      new THREE.MeshStandardMaterial({ color: 0xa0a0b0, roughness: 0.6 })
    );
    body.position.y = 1.5;
    grp.add(body);
    const roof = new THREE.Mesh(
...
function createEnemyMarker(x, y, z, etype) {
  ...
  if (etype === 'zombie') {
    const body = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x557744, roughness: 0.6 })
    );
    body.position.y = 0.75;
    grp.add(body);
    const head = new THREE.Mesh(
      new THREE.SphereGeometry(0.3, 8, 6),
      new THREE.MeshStandardMaterial({ color: 0x889966, roughness: 0.6 })
    );
    head.position.y = 1.7;
    grp.add(head);
  } else if (etype === 'hexapod') {
    const body = new THREE.Mesh(
      new THREE.TaperedBoxGeometry()
        ? new THREE.BoxGeometry(1.5, 0.6, 1.8)
        : new THREE.BoxGeometry(1.5, 0.6, 1.8),
      new THREE.MeshStandardMaterial({ color: 0x4a4a5a, roughness: 0.4 })
    );
    body.position.y = 0.8;
    grp.add(body);
    const dome = new THREE.Mesh(
      new THREE.SphereGeometry(0.2, 8, 6),
      new THREE.MeshStandardMaterial({ color: 0x6b6b7b, roughness: 0.3 })
    );
    dome.position.y = 1.2;
    grp.add(dome);
    // 6 leg stubs
    ...
    legPos.forEach(function (p) {
      var leg = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0x888866, roughness: 0.6 })
      );
...
function createWaypointMarker(x, y, z, idx) {
  ...
  const dot = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xf5954a, emissive: 0xf5954a, emissiveIntensity: 0.5 })
  );
  dot.position.y = 0.5;
  grp.add(dot);
...
function refreshPatrolLines(enemyId) {
  ...
  if (!ent || !ent.patrol || ent.patrol.length < 2) {
    if (patrolLines[enemyId]) {
      scene.remove(patrolLines[enemyId]);
      patrolLines[enemyId].geometry.dispose();
      patrolLines[enemyId].material.dispose();
      delete patrolLines[enemyId];
    }
    return;
  }
  ...
  patrolLines[enemyId] = new THREE.Line(
    geo,
    new THREE.LineBasicMaterial({ color: 0xf5954a, linewidth: 1, transparent: true, opacity: 0.7 })
  );
...
function addEntity(type, x, y, z, etype, yaw) {
  ...
  if (type === 'enemy') {
    ent.enemyType = etype || entityType;
    ent.patrol = [];
    ent.cfg = defaultEnemyCfg(ent.enemyType);
  }
  ...
  switch (type) {
    case 'spawn':
      marker = createSpawnMarker(x, y, z, yaw || 0);
      break;
    case 'tree':
      marker = createTreeMarker(x, y, z, ent.subType);
      break;
    case 'building':
      marker = createBuildingMarker(x, y, z, ent.subType, yaw || 0);
      break;
    case 'enemy':
      marker = createEnemyMarker(x, y, z, ent.enemyType);
      break;
  }
  if (marker) {
    scene.add(marker);
    entityMarkers[id] = marker;
  }
  refreshEntityList();
  return ent;
}
...
function changeEntityType(eid, newSubType) {
  ...
  // 重建标记
  if (entityMarkers[eid]) {
    scene.remove(entityMarkers[eid]);
    disposeGroup(entityMarkers[eid]);
    delete entityMarkers[eid];
  }
  let marker;
  switch (ent.type) {
    case 'tree':
      marker = createTreeMarker(ent.position.x, ent.position.y, ent.position.z, ent.subType);
      break;
    case 'building':
      marker = createBuildingMarker(ent.position.x, ent.position.y, ent.position.z, ent.subType);
      break;
    case 'enemy':
      marker = createEnemyMarker(ent.position.x, ent.position.y, ent.position.z, ent.enemyType);
      break;
  }
  if (marker) {
    scene.add(marker);
    entityMarkers[eid] = marker;
  }
  refreshEntityList();
}
...
function deleteEntity(id) {
  ...
  if (entityMarkers[id]) {
    scene.remove(entityMarkers[id]);
    disposeGroup(entityMarkers[id]);
    delete entityMarkers[id];
  }
  // 清理子标记（巡逻点）
  Object.keys(entityMarkers).forEach((k) => {
    if (k.startsWith(id + '_wp')) {
      scene.remove(entityMarkers[k]);
      disposeGroup(entityMarkers[k]);
      delete entityMarkers[k];
    }
  });
  if (patrolLines[id]) {
    scene.remove(patrolLines[id]);
    patrolLines[id].geometry.dispose();
    patrolLines[id].material.dispose();
    delete patrolLines[id];
  }
  if (highlightRings[id]) {
    scene.remove(highlightRings[id]);
    highlightRings[id].geometry.dispose();
    highlightRings[id].material.dispose();
...
function syncEnemyConfigPanel() {
  ...
  if (title) {
    title.innerHTML = batch
      ? `⚙️ 敌人属性 <span style="color:#f5954a;font-size:10px;">(批量编辑 ${nEnemy}个)</span>`
      : '⚙️ 敌人属性';
  }
  ...
  cfgModeBar.querySelectorAll('.cfg-mode-btn').forEach((b) => {
    ...
    if (mode === 'reactive')
      b.classList.toggle('active', refEnt.cfg.reactive === true && refEnt.cfg.aggressive !== true);
    else if (mode === 'aggressive') b.classList.toggle('active', refEnt.cfg.aggressive === true);
    else if (mode === 'none')
      b.classList.toggle('active', refEnt.cfg.reactive === false && refEnt.cfg.aggressive !== true);
  });
  ...
  if (!batch) {
    ...
    if (wpCount > 0) {
      cfgPatrolPrev.innerHTML =
        '<div style="color:#f5954a;margin-top:2px;">📍 巡逻点 (' +
        wpCount +
        '):</div>' +
        refEnt.patrol
          .map(
            (wp, i) =>
              `<div class="wp-line">${i + 1}. X:${wp.x.toFixed(1)} Z:${wp.z.toFixed(1)}</div>`
          )
          .join('');
    } else {
...
cfgModeBar.addEventListener('click', (ev) => {
  ...
  enemies.forEach((ent) => {
    if (!ent.cfg) ent.cfg = defaultEnemyCfg(ent.enemyType || 'assault');
    if (mode === 'reactive') {
      ent.cfg.reactive = true;
      ent.cfg.aggressive = false;
    } else if (mode === 'aggressive') {
      ent.cfg.reactive = false;
      ent.cfg.aggressive = true;
    } else {
      ent.cfg.reactive = false;
      ent.cfg.aggressive = false;
    }
...
function refreshEntityList() {
  ...
  if (mapData.entities.length === 0) {
    el.innerHTML = '<div class="empty-msg">暂无实体</div>';
  ...
  catOrder.forEach((type) => {
    ...
    html +=
      `<div class="cat-header${collapsed ? ' collapsed' : ''}" data-cat="${type}">` +
    ...
    ents.forEach((e) => {
      ...
      if (e.type === 'enemy')
        sub = ` (${enemyNames[e.enemyType] || e.enemyType} | 巡逻:${(e.patrol || []).length})`;
  ...
  if (nSel === 1) {
    ...
    if (selEnt && selEnt.type === 'tree') {
      html +=
        '<div style="margin-top:6px;font-size:10px;color:#888;">修改树种类:</div><div style="display:flex;gap:4px;margin:4px 0;">' +
        ['cone', 'sphere', 'oak']
          .map(
            (t) =>
              `<button class="chtype-btn${selEnt.subType === t ? ' active' : ''}" data-cht="${t}" style="background:#333350;color:#ccc;border:1px solid #4a4a65;border-radius:3px;padding:3px 6px;font-size:10px;cursor:pointer;">${treeNames[t]}</button>`
          )
          .join('') +
        '</div>';
    }
    if (selEnt && selEnt.type === 'building') {
      html +=
        '<div style="margin-top:6px;font-size:10px;color:#888;">修改建筑种类:</div><div style="display:flex;gap:4px;margin:4px 0;">' +
        ['bungalow', 'villa', 'apartment']
          .map(
            (t) =>
              `<button class="chtype-btn${selEnt.subType === t ? ' active' : ''}" data-cht="${t}" style="background:#333350;color:#ccc;border:1px solid #4a4a65;border-radius:3px;padding:3px 6px;font-size:10px;cursor:pointer;">${bldgNames[t]}</button>`
          )
          .join('') +
        '</div>';
    }
    if (selEnt && selEnt.type === 'enemy') {
      html +=
        '<div style="margin-top:6px;font-size:10px;color:#888;">修改敌人种类:</div><div style="display:flex;gap:4px;margin:4px 0;">' +
        ['assault', 'zombie', 'hexapod']
          .map(
            (t) =>
              `<button class="chtype-btn${selEnt.enemyType === t ? ' active' : ''}" data-cht="${t}" style="background:#333350;color:#ccc;border:1px solid #4a4a65;border-radius:3px;padding:3px 6px;font-size:10px;cursor:pointer;">${enemyNames[t]}</button>`
          )
          .join('') +
        '</div>';
    }
  ...
  if (nSel >= 2) {
    const selTypes = new Set(
      [...selectedEntityIds].map((id) => (mapData.entities.find((e) => e.id === id) || {}).type)
    );
    const selEnemies = [...selectedEntityIds].filter(
      (id) => (mapData.entities.find((e) => e.id === id) || {}).type === 'enemy'
    );
    html +=
      '<div style="margin-top:4px;font-size:10px;color:#f5954a;">已选 ' +
      nSel +
      ' 个实体 (类型:' +
      [...selTypes].join(',') +
      ')</div>';
    if (selEnemies.length > 0) {
      html +=
        '<div style="margin-top:4px;font-size:10px;color:#888;">批量修改敌人种类:</div><div style="display:flex;gap:4px;margin:4px 0;">' +
        ['assault', 'zombie', 'hexapod']
          .map(
            (t) =>
              `<button class="chtype-btn" data-cht="${t}" style="background:#333350;color:#ccc;border:1px solid #4a4a65;border-radius:3px;padding:3px 6px;font-size:10px;cursor:pointer;">${enemyNames[t]}</button>`
          )
          .join('') +
        '</div>';
      html +=
        '<div style="display:flex;gap:4px;margin:4px 0;"><button class="batch-btn" data-batch="copy-patrol">📋巡逻点复制到选中敌人</button><button class="batch-btn danger" data-batch="clear-patrol">🗑️清空选中巡逻点</button></div>';
    }
  }
  ...
  el.querySelectorAll('.entity-row').forEach((row) => {
    ...
    row.addEventListener('click', (ev) => {
      if (
        ev.target.classList.contains('btn-del') ||
        ev.target.classList.contains('chtype-btn') ||
        ev.target.classList.contains('batch-btn')
      )
        return;
      ...
      if (clickTimer) {
        ...
        if (ev.altKey) {
          mapData.entities.forEach((e) => {
            if (e.type === ent.type) selectedEntityIds.add(e.id);
          });
          overlayInfo.textContent =
            '✅ 已选中全部 ' + labels[ent.type] + ' (' + selectedEntityIds.size + '个)';
        } else {
          const subKey =
            ent.type === 'tree'
              ? 'subType'
          ...
          if (subKey) {
            mapData.entities.forEach((e) => {
              if (e[subKey] === ent[subKey]) selectedEntityIds.add(e.id);
            });
          } else {
            mapData.entities.forEach((e) => {
              if (e.type === ent.type) selectedEntityIds.add(e.id);
      ...
      clickTimer = setTimeout(() => {
        ...
        if (ev.ctrlKey || ev.metaKey) {
          if (selectedEntityIds.has(eid)) selectedEntityIds.delete(eid);
          else selectedEntityIds.add(eid);
...
function refreshGroupList() {
  ...
  if (!mapData.groups || mapData.groups.length === 0) {
    el.innerHTML = '<div class="empty-msg">暂无编组</div>';
    return;
  }
  el.innerHTML = mapData.groups
    .map(
      (g) =>
  ...
  el.querySelectorAll('.group-row').forEach((row) => {
    row.addEventListener('click', (ev) => {
      ...
      if (g) {
        selectedEntityIds.clear();
        g.entityIds.forEach((id) => selectedEntityIds.add(id));
        refreshEntityList();
      }
    });
  });
...
function batchCopyPatrol() {
  ...
  if (selEnemies.length < 2) {
    alert('需要至少选中2个敌人');
    return;
  }
  const src = mapData.entities.find((e) => e.id === selEnemies[0]);
  if (!src || !src.patrol || src.patrol.length === 0) {
    alert('第一个选中的敌人没有巡逻点');
    return;
  }
...
function batchClearPatrol() {
  ...
  selEnemies.forEach((eid) => {
    ...
    if (patrolLines[eid]) {
      scene.remove(patrolLines[eid]);
      patrolLines[eid].geometry.dispose();
      patrolLines[eid].material.dispose();
      delete patrolLines[eid];
    }
  });
  refreshEntityList();
...
function setEntityMode(mode) {
  if (entityMode === mode) {
    ...
    selectedEntityIds.clear();
    Object.values(highlightRings).forEach((r) => {
      scene.remove(r);
      r.geometry.dispose();
      r.material.dispose();
    });
  ...
  brushMode = 'cursor';
  ['cursor', 'raise', 'lower', 'smooth', 'paint'].forEach((k) => {
    const b = document.getElementById('btn-' + k);
    if (b) b.classList.toggle('active', k === 'cursor');
  });
  ...
  ['select', 'spawn', 'tree', 'building', 'enemy', 'waypoint', 'bridge'].forEach((k) => {
    const b = document.getElementById('btn-entity-' + k);
    if (b) b.classList.toggle('active', entityMode === k);
  });
  ...
  document
    .getElementById('entity-sel-building')
    .classList.toggle('visible', entityMode === 'building');
  if (entityMode) {
    const names = {
      select: '选择实体',
      spawn: '放置出生点',
    ...
    overlayInfo.textContent =
      entityMode === 'select'
        ? '👆 点击地图上实体选中，拖拽移动 | 右键平移'
        : '📍 ' + names[entityMode] + ' — 点击地图放置';
  }
}

document
  .getElementById('btn-entity-select')
  .addEventListener('click', () => setEntityMode('select'));
...
document
  .getElementById('btn-entity-building')
  .addEventListener('click', () => setEntityMode('building'));
document.getElementById('btn-entity-enemy').addEventListener('click', () => setEntityMode('enemy'));
document
  .getElementById('btn-entity-waypoint')
  .addEventListener('click', () => setEntityMode('waypoint'));
document.getElementById('btn-entity-bridge').addEventListener('click', () => {
  setEntityMode('bridge');
  bridgePt1 = null;
});
