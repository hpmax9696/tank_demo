...
function createRoadMeshes(roadSegments, targetScene) {
    ...
    function buildRoadStrip(pts, width, material, offsetY) {
        ...
        const smoothH = rawH.map((h, i) => {
            const j2 = Math.max(0, i-2), j1 = Math.max(0, i-1), k1 = Math.min(n-1, i+1), k2 = Math.min(n-1, i+2);
        ...
        for (let i = 0; i < n - 1; i++) {
            ...
            const x0 = pts[i].x - ux * ext, z0 = pts[i].z - uz * ext;
            const x1 = pts[i+1].x + ux * ext, z1 = pts[i+1].z + uz * ext;
            const th0 = smoothH[i], th1 = smoothH[i+1];
            topV.push(x0+snx*hw, th0, z0+snz*hw, x0-snx*hw, th0, z0-snz*hw, x1+snx*hw, th1, z1+snz*hw, x1-snx*hw, th1, z1-snz*hw);
            botV.push(x0+snx*hw, th0-ROAD_THICKNESS, z0+snz*hw, x0-snx*hw, th0-ROAD_THICKNESS, z0-snz*hw, x1+snx*hw, th1-ROAD_THICKNESS, z1+snz*hw, x1-snx*hw, th1-ROAD_THICKNESS, z1-snz*hw);
        }
        const topIdx = [], botIdx = [];
        for (let i = 0; i < n - 1; i++) {
            const a = i * 4;
        ...
        topMesh.receiveShadow = true; topMesh.userData.isRoad = true;
        targetScene.add(topMesh); _roadMeshes.push(topMesh);
        const botMesh = new THREE.Mesh(botGeo, material);
        botMesh.receiveShadow = true; botMesh.userData.isRoad = true;
...
window.getRoadSurfaceY = function(px, pz) {
    ...
    for (const seg of segs) {
        ...
        let t, cx, cz;
        if (lenSq < 0.001) { t = 0; cx = seg.x1; cz = seg.z1; }
        else {
            t = ((px - seg.x1)*dx + (pz - seg.z1)*dz) / lenSq;
            t = Math.max(0, Math.min(1, t));
            cx = seg.x1 + dx*t; cz = seg.z1 + dz*t;
        }
