...
var _hexaAnimNames = [
  ...
  '4/23 步行后退 (Walk Back)',
  '5/23 奔跑后退 (Run Back)',
  '6/23 左平移 (Strafe Left)',
  '7/23 右平移 (Strafe Right)',
  '8/23 静态左转 (Static Turn L)',
  '9/23 静态右转 (Static Turn R)',
  '10/23 步行左转 (Walk Turn L)',
  '11/23 步行右转 (Walk Turn R)',
  '12/23 平移左转 (Strafe Turn L)',
  '13/23 平移右转 (Strafe Turn R)',
  '14/23 后退左转 (Walk Back Turn L)',
  '15/23 后退右转 (Walk Back Turn R)',
  '16/23 奔跑左转 (Run Turn L)',
  '17/23 奔跑右转 (Run Turn R)',
  '18/23 奔退左转 (Run Back Turn L)',
  '19/23 奔退右转 (Run Back Turn R)',
  '20/23 奔跑左平移 (Strafe Run L)',
  '21/23 奔跑右平移 (Strafe Run R)',
  '22/23 受击踉跄 (Stagger)',
  '23/23 死亡 (Death)',
];
var _hexaAnimDurations = [
  3500, 1500, 800, 1600, 900, 1700, 1700, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 1200,
  1200, 1300, 1300, 1000, 1000, 1100, 2500,
];
...
function collectRefs() {
  var hexRoot = null;
  M.modelRoot.children.forEach(function (c) {
    if (c.name === '六足战车') hexRoot = c;
  });
  if (!hexRoot) return;
  ...
  for (var wi = 0; wi < weaponNames.length; wi++) {
    ...
    hexRoot.traverse(function (n) {
      ...
      if (n.name === weaponNames[wi].replace('加特林', '加特林支架').replace('导弹巢', '导弹支架'))
        mount = n;
    });
    if (wg && mount) {
      weaponRefs.push({
  ...
  _animRefs = M._animRefs = {
    legs: ctx.legs.map(function (l) {
      return {
        thighPivot: l.thighPivot,
        shinPivot: l.shinPivot,
        anklePivot: l.anklePivot,
        thighGroup: l.thighPivot.parent.parent,
        restThighX: l.restThigh,
        restShinX: l.restShin,
        restAnkleX: l.restAnkle,
        restLegY: l.restHip,
        tipLocal: l.tipLocal,
        tipWorld: l.tipLocal.clone().applyMatrix4(l.anklePivot.matrixWorld),
        _initFootDist: l._initFootDist,
        _shinSign: l._shinSign,
        _yLimit: l._yLimit,
        plantPos: l.plantPos,
        swingFrom: l.swingFrom,
        swingTo: l.swingTo,
      };
    }),
    hexRoot: hexRoot,
...
function updateFrame(dt, t, elapsed, duration, animIndex) {
  ...
  // 动画切换检测
  var switched =
    (_animRefs._lastDuration !== undefined && _animRefs._lastDuration !== duration) ||
...
function _restartIKTest() {
  ...
  ikTestActive = false;
  if (ikTestAnimId) {
    cancelAnimationFrame(ikTestAnimId);
    ikTestAnimId = null;
  }
  var sc = getScene();
  for (var i = sc.children.length - 1; i >= 0; i--) {
    var c = sc.children[i];
    if (c.name && c.name.indexOf('_ik') === 0) {
      sc.remove(c);
  ...
  if (rp.hexRoot) {
    ...
    rp.hexRoot.children.forEach(function (child) {
      ...
      child.traverse(function (n) {
        if (n.userData._ikWasVisible !== undefined) {
          n.visible = n.userData._ikWasVisible;
          delete n.userData._ikWasVisible;
        }
      });
    });
  }
...
function _ensureIKSelectors() {
  ...
  if (!btn) return;
  var css =
    'margin-left:4px;padding:2px 4px;font-size:11px;background:#222;color:#ccc;border:1px solid #555;border-radius:3px;';
  var legSel = document.createElement('select');
  legSel.id = 'ik-leg-select';
  legSel.style.cssText = css;
  legSel.innerHTML =
    '<option value="左前腿">左前腿</option><option value="左中腿">左中腿</option><option value="左后腿">左后腿</option>';
  ...
  modeSel.innerHTML =
    '<option value="1">Y轴下蹲</option><option value="2">X轴左右</option><option value="3">Z轴前后</option>';
  ...
  if (ikTestActive) {
    ...
    hexRoot.children.forEach(function (child) {
      if (child.name !== IK_TEST_LEG) {
        child.traverse(function (n) {
          if (n !== child && n.visible !== undefined) n.userData._ikWasVisible = n.visible;
        });
        child.visible = false;
      }
    });
    ...
    var hipW = new THREE.Vector3();
    thighPivot.getWorldPosition(hipW);
    var ankleW = new THREE.Vector3();
    anklePivot.getWorldPosition(ankleW);
    ...
    function ikLoop(now) {
      ...
      if (mode === 1)
        hexRoot.position.set(
          ikTestRestPoses.hexRootX,
          ikTestRestPoses.hexRootY + wave,
          ikTestRestPoses.hexRootZ
        );
      else if (mode === 2)
        hexRoot.position.set(
          ikTestRestPoses.hexRootX + wave,
      ...
      sp.rotation.x += kneeBias * 0.1;

      for (var iter = 0; iter < 40; iter++) {
        hexRoot.updateMatrixWorld(true);
        var tW = new THREE.Vector3();
        tp.getWorldPosition(tW);
        var eWt = _tipW();
        ...
        if (lt > 0.0003) {
          axt.normalize();
          tp.rotation.x += Math.atan2(lt, dEt.dot(dTt)) * axt.dot(_worldX(tp)) * 0.5;
        }
        hexRoot.updateMatrixWorld(true);
        ...
        if (ls > 0.0003) {
          axs.normalize();
          sp.rotation.x += Math.atan2(ls, dEs.dot(dTs)) * axs.dot(_worldX(sp)) * 0.5;
        }
        hexRoot.updateMatrixWorld(true);
        var hW = new THREE.Vector3();
        tp.getWorldPosition(hW);
        var eWh = _tipW();
        ...
        if (lh > 0.0003) {
          axh.normalize();
          lp.rotation.y += Math.atan2(lh, dEh.dot(dTh)) * axh.dot(_worldY(lp)) * 0.5;
        }
      }

      hexRoot.updateMatrixWorld(true);
      var hwD = new THREE.Vector3();
      tp.getWorldPosition(hwD);
      var twD = _tipW();
      var awD = new THREE.Vector3();
      ap.getWorldPosition(awD);
...
function _turnTestUpdateDot(name, pos) {
  ...
  for (var i = 0; i < sc.children.length; i++) {
    if (sc.children[i].name === name) {
      sc.children[i].position.copy(pos);
      return;
    }
  }
}
function _turnTestCleanupDots() {
  ...
  for (var i = sc.children.length - 1; i >= 0; i--) {
    var c = sc.children[i];
    if (c.name && c.name.indexOf('_tt') === 0) {
      sc.remove(c);
      if (c.geometry) c.geometry.dispose();
      if (c.material) c.material.dispose();
    }
...
function toggleHexTurnTest() {
  ...
  if (turnTestActive) {
    ...
    hexRoot.children.forEach(function (child) {
      if (keepNames.indexOf(child.name) < 0) {
        turnTestRestPoses.hiddenParts.push(child);
        child.visible = false;
      }
    });
    var bodyGroup = null;
    hexRoot.children.forEach(function (c) {
      if (c.name === '车体') bodyGroup = c;
    });
    if (bodyGroup) {
      bodyGroup.children.forEach(function (c) {
        if (c.name !== '下车体') {
          c.visible = false;
          turnTestRestPoses.hiddenParts.push(c);
        }
      });
    }
    ...
    for (var li = 0; li < prefixes.length; li++) {
      ...
      turnTestState.legs.push({
        prefix: prefix,
        thighPivot: thighInfo.rotTarget,
        shinPivot: shinInfo.rotTarget,
        anklePivot: ankleInfo.rotTarget,
        thighGroup: thighInfo.group,
    ...
    turnTestState._tmpCtx = {
      root: hexRoot,
      legs: turnTestState.legs.map(function (l) {
        return {
          thighPivot: l.thighPivot,
          shinPivot: l.shinPivot,
          anklePivot: l.anklePivot,
          hipJoint: l.thighGroup.parent,
          tipLocal: l.tipLocal,
          restAnkle: l.restAnkleX,
          restHip: l.restLegY,
          _shinSign: 1,
          _yLimit: 0.7,
        };
    ...
    if (turnTestAnimId) {
      cancelAnimationFrame(turnTestAnimId);
      turnTestAnimId = null;
    }
    ...
    if (rp.hiddenParts)
      rp.hiddenParts.forEach(function (c) {
        c.visible = true;
      });
...
function _turnTestLoop(now) {
  ...
  hexRoot.getWorldPosition(bodyC);
  ...
  for (var i = 0; i < turnTestState.legs.length; i++) {
    ...
    if (inStance) {
      ...
      CORE._ccdLeg(
        turnTestState._tmpCtx,
        turnTestState._tmpCtx.legs[i],
        leg.plantPos,
        20 + Math.round(Math.abs(turnRate) * 13),
        0.8
      );
    } else {
      if (leg._wasStance) {
...
M._hexTurnActive = function () {
  return turnTestActive;
};
...
function toggleWeaponCalibrate() {
  ...
  hexRoot.traverse(function (node) {
    ...
    if (bi >= 0) {
      ...
      barrelDirs[bi] = new THREE.Vector3(0, 1, 0).applyQuaternion(wq).normalize();
    }
  });
  ...
  for (var ni = 0; ni < _WC_MOUNT_NAMES.length; ni++) {
    ...
    var gpWorldPos = new THREE.Vector3();
    grandParent.getWorldPosition(gpWorldPos);
    var gpWorldQuat = new THREE.Quaternion();
    grandParent.getWorldQuaternion(gpWorldQuat);
  ...
  if (weaponCalData.muzzles.length === 0) {
    weaponCalActive = false;
  ...
  if (gatSlider) {
    gatSlider.value = 0;
    gatSlider.oninput = function () {
      _onCalSlider('gatling', this.value);
    };
  }
  if (misSlider) {
    misSlider.value = 0;
    misSlider.oninput = function () {
      _onCalSlider('missile', this.value);
    };
  }
}
...
function _onCalSlider(type, valDeg) {
  ...
  if (type === 'gatling') {
    ...
    var el = document.getElementById('weaponcal-val-gat');
    if (el) el.textContent = deg + '°';
  } else {
    weaponCalData.misPitch = rad;
    var el2 = document.getElementById('weaponcal-val-mis');
...
function _updateOneMuzzleLine(m, pitchRad) {
  var cosA = Math.cos(pitchRad),
    sinA = Math.sin(pitchRad);
...
function _weaponCalCleanup() {
  ...
  for (var i = sc.children.length - 1; i >= 0; i--) {
    var c = sc.children[i];
    if (c.name && c.name.indexOf('_wc_') === 0) {
      sc.remove(c);
      if (c.geometry) c.geometry.dispose();
      if (c.material) c.material.dispose();
    }
