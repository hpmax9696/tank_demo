...
> **当前版本：v0.33.1** | 基于 Three.js 的多模块 3D 浏览器游戏 + 地图编辑器
...
2. **拖拽河流**：宽度用 `brushRadius` 替代硬编码10，深度用 `brushStrength` 替代硬编码3
...
### v0.26.3 — 火焰伤害跳距模型修复（2026-05-10）
**修复**：火焰视觉射程/速度与伤害模型统一，跳距模型替代elapsed/propDelay，修复第5跳丢失，近防机枪+av-02补充部署。
**已知问题**：av-01初始不动、av-02无地形俯仰、首战机枪音效翻倍、av-02仇恨不共享（待v0.26.4）。
...
### v0.25.4 — 双人阴影修复（2026-05-09）
**修复**：双人模式阴影跟随两玩家中点，按间距动态扩展覆盖范围。

### v0.25.3 — 轨迹计入地形俯仰（2026-05-09）
**修复**：发射计算terrainPitch，总仰角=地形+加速+后坐+5.7°基础，平地射程~200u。

### v0.25.2 — 炮弹轨迹+焦痕修复（2026-05-09）
**修复**：炮弹水平速度不变，基础仰角1.7°；焦痕法线采样+depthOffset加强。

### v0.25.1 — 俯仰发射角+焦痕贴坡（2026-05-09）
**修复**：炮弹发射计入坦克俯仰，焦痕贴合地形坡度。

### v0.25.0 — 地面命中焦痕+碎片（2026-05-09）
**新功能**：炮弹着地产生音效+土块飞溅+焦痕(3秒渐消)。

### v0.24.10 — 阴影降级512+36m跟随（2026-05-09）
**优化**：阴影类型PCFSoft→PCFShadow+512分辨率+相机36m跟随坦克，渲染从27ms→13.5ms。

### v0.24.9 — 四阶段探针+遮挡池优化（2026-05-09）
**诊断**：四阶段性能探针实时显示物理/战斗/杂项/渲染耗时。**优化**：透明材质池消除clone/dispose循环。

### v0.24.8 — 半透明共享Bug修复（2026-05-09）
**修复**：material.clone()隔离半透明材质，恢复时dispose还原。阴影贴图2048→1024，草丛关投射阴影。

### v0.24.7 — 外围精简+遮挡降频（2026-05-09）
**优化**：移除外围障碍物+遮挡检测降频0.3s+8m预过滤，性能恢复60fps。


### v0.24.6 — 河流300m+遮挡半透明（2026-05-09）
**修复**：河流扩展300m+障碍物遮挡半透明(opacity=0.3)。

### v0.24.5 — 移除天空系统+调整LOD（2026-05-09）
**重构**：移除Sky.js+OBS_RADIUS 65→90+FOV 45°。
...
- 树干6→16段、锥形冠8→24段、球形冠8×6→20×14段
- 新增橡树品种（椭球形树冠），树种2→3，视觉多样性显著提升
- 预期draw calls: 810→~322（-60%），性能余量: 3ms→5-6ms
