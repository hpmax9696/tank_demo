...
> **当前版本：v0.50.0** | 基于 Three.js 的多模块 3D 浏览器游戏 + 地图编辑器
...
### v0.50.0 — 双管线村落生成系统+CDP自动验证（2026-06-01）

- **双管线架构**: 管线A(仅地形+自动平整)和管线B(道路+村落+障碍物)可独立调用，中间可手画河流
- **三按钮**: 🎲一键全部 / 🌍仅生成地形 / 🏘️生成道路与村落，替换旧的两个独立按钮
- **自动平整**: 管线A内建保峰压谷，保留N个山峰，谷削至配置比例，确保可建面积≥60%
- **全图掩码网格**: Uint8Array位掩码(MG_BUILDABLE/FORBIDDEN/WATER/ROAD等)，一条位运算解决所有碰撞
