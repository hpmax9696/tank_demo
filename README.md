...
> **当前版本：v0.42.1** | 基于 Three.js 的多模块 3D 浏览器游戏 + 地图编辑器
...
- **树木销毁修复**：恢复 disposeTreeInstance() 函数，炮弹击碎树木后正确消失
- **倒车转弯修复**：在 input.js 中添加前进/后退履带速度交换逻辑，解决倒车转弯方向反问题
- **弹种切换音效**：切换 AP/HE 弹时播放提示音效
- **弹种持久化修复**：修复弹种选择在游戏重启后保持为上次选择的问题
- **弹种切换触发装填**：切换弹种时自动触发重新装填

### v0.39.1 — 瞄准+操控+预测线全面修复（2026-07-25）
- **瞄准重写**：世界方向→坦克本地四元数逆变换，正确处理 pitch+roll+yaw，陡坡精准
- **rotation.order = 'YXZ'**：地形俯仰在坦克自身坐标系中生效，坡上不再扎地/翘头
- **准星四阶段判定**：障碍物射线→抛物线地形采样→shellR 0.18m体积容差→目标障碍物跳回
- **树木 InstancedMesh 加入 obstacleMeshes**：射线检测/瞄准/遮挡全面覆盖树木
- **建筑恢复**：Poisson 采样 15% 分配随机建筑（锥形35%/球形30%/橡树20%）
- **弹道预测线强化**：全模式启用 + 抛物线公式 + 地形截断 + 障碍物 shellR 边缘 + 敌人坦克截断
- **手柄全面修复**：双轴映射（axes[2/4]=X, axes[3/5]=Y）+ 粘滞切换（WASD/鼠标才回键鼠）+ 光标自动隐藏
- **快速开炮修复**：mouseFireRequested 锁存，点击再快也不丢
