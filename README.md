...
> **当前版本：v0.24.1** | 基于 Three.js 的多模块 3D 浏览器游戏
...
2. 当前版本 console.log：`🎮 坦克运动demo v0.24.1 | 纹理覆盖300×300 | 外圈自然平展 | 01a/01b远景可见`
3. 页面右上角有调试信息（版本号/FPS/里程/坦克坐标/可见障碍物数量）
4. 修改代码后 `Ctrl+F5` 强制刷新，或关闭标签页重新访问 localhost 确保不使用缓存

### 代码规模（截至 v0.24.1）
- `index.html`：约 3565 行（HTML + CSS + JS 游戏引擎，含地形扩展300×300+纹理全尺寸覆盖+Preetham天空+草丛系统）
