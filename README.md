# 🎮 坦克运动 Demo — 3D 坦克对战游戏

> **当前版本：v0.18.3** | 基于 Three.js 的多模块 3D 浏览器游戏
> 支持单人探索和本地双人对战（1P 键盘 + 2P 手柄）。
> 双击 `index.html` 即可运行，无需服务器。

---

## 一、快速开始

### 运行方式
在任意现代浏览器（Chrome / Edge / Firefox）中**双击 `index.html`** 即可直接运行。支持 `file://` 协议，无需启动本地服务器。

### 操作说明

| 模式 | 操作 |
|------|------|
| 单人 | 1P: W/S (左履带) ↑/↓ (右履带) 空格 (开炮) ESC (返回) |
| 双人 | 1P键盘 + 2P手柄 (左右摇杆 + RT开炮) |
| 模型预览 | 鼠标拖拽旋转视角 / 滚轮缩放 / 下拉菜单切换模型 / ESC返回 |

- **履带独立控制**：差速驱动（两侧履带速差产生转向）
- **手柄摇杆**：3 档速度（>80% → 全速，>50% → 2/3速，>20% → 1/3速）

---

## 二、技术栈

| 技术 | 版本/说明 |
|------|-----------|
| Three.js | 0.160.0 (UMD 构建，本地加载) |
| 粒子系统 | fireSmokeParticles.js（火焰/烟雾/爆炸效果） |
| 音频 | Web Audio API（程序化生成，无外部音频文件） |
| 部署 | `file://` 协议直接运行，无需网络（所有依赖本地化） |

---

## 三、项目结构

```
坦克对战demo/
├── index.html             # 主入口文件（HTML + CSS + JS 游戏引擎，模型逻辑已拆分至 models/）
├── three.min.js           # Three.js 库（UMD 构建，~654KB，本地加载）
├── fireSmokeParticles.js  # 粒子系统模块（火焰/烟雾/爆炸效果）
├── README.md              # 本文件
└── models/                # 模型文件夹
    ├── modelRegistry.js   # 模型注册表（统一管理+权重随机）
    ├── tank.js            # 坦克模型（绿色/沙漠色）
    ├── t34-85.js          # T-34-85 后期型高精度模型（绿色/沙漠色）
    ├── trees.js           # 树木模型（锥形树/球形树）
    ├── buildings.js       # 建筑模型（平房/别墅/公寓）
    └── windmill.js        # 风车磨坊模型（PlaneGeometry叶片）
```

### 代码结构

```
index.html:
  ├── HTML: #menu-overlay（主菜单）、#game-container（游戏容器）
  │       ├── 主菜单：标题 + 版本号 + 模式选择 + 更新日志
  │       ├── HUD：返回按钮 + 操作提示条
  │       ├── 双人：分屏分隔线 + 对战结果覆盖层 + 方向指示箭头
  │       └── 血条/装填条：Canvas 平面（billboard 始终面向摄像机）
  ├── CSS：响应式菜单、半透明控件、箭头 UI
  └── JS：完整游戏引擎，见下方模块分解

fireSmokeParticles.js:
  ├── DamageEffects 类 — 坦克受伤时的持续火焰烟雾（40火焰 + 30烟雾）
  └── ExplosionEffects 类 — 坦克死亡时的一次性爆炸效果（120火焰 + 80烟雾）
      ├── 火焰粒子：白色/黄色/橙色/红色，AdditiveBlending 混合
      ├── 烟雾粒子：灰色，NormalBlending 混合
      └── 渐隐机制：基于最小生命周期比例平滑淡出
```

### JS 模块分解（index.html）

```
├── DOM 引用 & 状态变量
├── 音频系统（engine / fire / explosion / hit / debris）
├── 状态机（gameMode: menu | single | versus）
├── 键盘/手柄输入处理
├── 玩家工厂（createPlayer）
├── 场景初始化（渲染器 / 光照 / 地面 / 坦克 / 障碍物 / 摄像机）
├── 地面系统（200×200 Canvas 网格纹理）
├── T-34/85 坦克模型（v6.1 高精度，单人/双人对战使用）+ 简化坦克（备选）
├── 障碍物系统（泊松盘采样 + 6种建筑/树木 + LOD可见性管理）
├── 运动物理（差速驱动 / 加速制动惯性 / 碰撞检测 / 俯仰效果）
├── 火炮系统（炮弹 / 炮口焰 / 碎片 / 命中火花）
├── 坦克爆炸系统（大型火焰烟雾 + 殉爆连锁反应）
├── 殉爆系统（坦克爆炸引爆近距离障碍物，3.5米半径）
├── 游戏循环（gameLoop / versusGameLoop）
├── 摄像机（第三人称追尾视角 + 双人分屏）
├── 双人对战系统（分屏渲染 / 血条 / 装填条 / 胜负判定）
├── 指向箭头系统（透视投影 + 屏幕坐标映射 + behind检测）
└── 事件绑定（resize / ESC 返回菜单）
```

---

## 四、游戏系统详解

### 4.1 世界参数

| 参数 | 值 | 说明 |
|------|-----|------|
| 世界大小 | 200×200 单位 | ±100 边界空气墙 |
| 障碍物数量 | 350 | 泊松盘采样，最小间距 6 单位 |
| 出生安全区 | 半径 10 单位 | 中心无障碍物 |
| 障碍物可见距离 | 65 单位 | 超出则隐藏（性能优化） |

### 4.2 障碍物类型

| 类型 | 比例 | 碰撞半径 | 说明 |
|------|------|----------|------|
| 锥形树 | 35% | 0.45 | 0.7~1.6 高度随机 |
| 球形树 | 35% | 0.45 | 0.7~1.4 高度随机 |
| 平房 | 10% | 0.55 | 带锥形屋顶 |
| 别墅 | 10% | 0.55 | 带窗户 |
| 公寓 | 7% | 0.55 | 带窗户阵列 |
| 风车 | 3% | 0.50 | 4片旋转叶片 |

### 4.3 坦克物理

| 参数 | 值 | 说明 |
|------|-----|------|
| 履带间距 | 3.2 | 差速转向基准 |
| 最高速度 | 4.0 单位/秒 | |
| 加速率 | 6.0 | 同向加速 |
| 制动率 | 10.0 | 反向制动（急刹） |
| 惯性滑行 | 3.5 | 松手后自然减速 |
| 碰撞半径 | 0.55 | 半宽 |
| 俯仰增益 | 0.12 | 加减速车体倾斜 |
| 最大俯仰 | 0.18 rad | ≈10° |

### 4.4 炮弹 & 战斗系统

| 参数 | 值 | 说明 |
|------|-----|------|
| 初速 | 22.0 单位/秒 | 水平分量 |
| 初始上扬 | 0.3 | 垂直初速（vy），低弹道 |
| 重力 | 1.0 | 平射弹道 |
| 最大射程 | 300 单位 | 超出自毁 |
| 装填时间 | 2.0 秒 | |
| 伤害 | 20 HP | 5炮击杀（100 HP） |
| 碎片数 | 12 | 命中障碍物产生碎块 |
| 碎片寿命 | 3.0 秒 | 渐隐消失 |
| 殉爆半径 | 3.5 米 | 坦克爆炸触发障碍物连锁爆炸 |

### 4.5 坦克爆炸效果（v0.13.5+）

- **受伤效果**（HP < 50）：坦克燃起火焰 + 散发烟雾（持续粒子系统）
  - 40个火焰粒子 + 30个烟雾粒子
- **死亡爆炸**（HP = 0）：大型爆炸火焰烟雾（一次性粒子爆发）
  - 120个火焰粒子 + 80个烟雾粒子
  - 粒子从中心向四周爆发，持续扩散至自然消散
  - 渐隐透明度基于 `minLifeRatio^0.5`（火焰）/ `minLifeRatio^0.3`（烟雾）

### 4.6 殉爆系统（v0.13.6+）

- 坦克死亡爆炸后，3.5米半径内的障碍物被引爆
- 障碍物被殉爆时产生碎块散开效果（不产生火焰）
- 防止无限递归：殉爆链内部爆炸传入 `skipChain=true`

### 4.7 音频系统

全部使用 Web Audio API 程序化生成，无需外部音频文件：

| 声音 | 技术 | 描述 |
|------|------|------|
| 引擎声 | 双锯齿波振荡器 (35+70Hz) | 频率随速度变化 |
| 履带声 | 白噪声 → 带通滤波 (600Hz) | 音量随速度变化 |
| 开炮声 | 锯齿波扫频 (120→25Hz) + 爆鸣噪声 | |
| 爆炸声 | 白噪声包络 → 低通滤波 (800→80Hz) | 0.6秒 |
| 命中声 | 方波扫频 (600→120Hz) | 0.25秒 |
| 碎片声 | 6个随机三角波 (600~2400Hz) | 0.07秒各 |

### 4.8 摄像机

- **类型**：第三人称追尾视角
- **FOV**：55°
- **位置**：坦克后方 14 单位，上方 10 单位
- **注视点**：坦克上方 3.5 单位（坦克在画面下 1/3 处）
- **分屏时**：左半屏用 `camera`（P1 视角），右半屏用 `camera2`（P2 视角）

### 4.9 双人分屏渲染流程

```
每帧：
1. reset viewport/scissor → 全屏（清除上一帧残留）
2. 更新两个玩家的物理
3. 左半屏：viewport(0, 0, halfW, fbH) + scissor → render(camera)
4. 右半屏：viewport(halfW, 0, rightW, fbH) + scissor → render(camera2)
5. reset scissor test → false
6. 更新指向箭头
```

### 4.10 指向箭头系统

- **显示条件**：两玩家距离 > 25 单位
- **隐藏条件**：距离 < 20 单位（滞后避免闪烁）
- **P1箭头**：P1 位置投影到左半屏，指向 P2 的屏幕方向
- **P2箭头**：P2 位置投影到右半屏，指向 P1 的屏幕方向
- **Behind 处理**：当目标在摄像机后方时（`project().z > 1`），反转方向向量并钳制到屏幕边缘

### 4.11 双人对战规则

- 初始位置：P1 (–15, 0) 绿色坦克，P2 (15, 0) 沙漠色坦克
- 初始朝向：面对面（P1 朝 +Z，P2 朝 –Z）
- HP：100，每炮 20 伤害，5 炮击杀
- 死亡特效：坦克爆炸火焰 + 碎片 + 障碍物殉爆
- 受伤特效：命中火花粒子 + 金属撞击声
- 同队碰撞：两坦克互相有物理碰撞
- 2秒后自动显示胜负结果

---

## 五、完整版本历史

### v0.18.3 — canvas尺寸管理重构，适配高DPI（2026-05-06）
**修复**：
- **canvas尺寸管理重构**：全部 `setSize` 调用增加第三个参数 `false`（阻止Three.js覆盖CSS），改用 `gameContainer.clientWidth/Height` 替代 `window.innerWidth/innerHeight`
- **CSS统一**：所有渲染器 canvas 统一使用 `position:absolute;top:0;left:0;width:100%;height:100%;display:block`，由容器决定实际像素尺寸
- 消除高DPI显示器（2K/4K）下canvas buffer与CSS渲染尺寸不匹配导致的画面偏移

### v0.18.2 — 修复高DPI分辨率下的viewport偏差（2026-05-06）
**修复**：
- **viewport计算修正**：统一使用 `renderer.domElement.width/height`（canvas实际物理像素）替代 `window.innerWidth * devicePixelRatio`（浮点数），消除高DPI显示器（2K/4K）下双人分屏偏移和单人模式坦克居中偏差

### v0.18.1 — T-34/85 模型尾部 + 履带修正（2026-05-06）
**修正**：
- **车体尾部**：从 4 点垂直方尾改为 **5 点倾斜剖面**（`REAR_OVERHANG=0.06`），新增 -35° 后倾尾板装甲板
- **发动机舱顶盖**：缩窄（`HULL_W-0.10`）+ 微前倾 4°，非平铺大盒子；散热格栅改为单排 9 条细条
- **履带系统重写**：
  - 底部/顶部：动态密集小段（`segL=0.095`）按实际距离均布，消除间隙
  - 前弧（诱导轮）：改用 `TorusGeometry` 半圆环（连续无断点）
  - 后弧（主动轮）：改用 `TorusGeometry` 半圆环（完全连续）

### v0.18.0 — T-34/85 模型 v6 真实照片对标重构（2026-05-06）
**基于用户提供的真实 T-34/85 照片进行全面重构**：
- **炮塔**：从六角棱柱+巨大半球穹顶（像房子）→ **LatheGeometry** 10 点铸造轮廓曲线（36 段旋转体），圆润"胖蘑菇"形，无大穹顶
- **防盾**：从方块粘在前面 → **变形 Sphere**(scale 1.1×0.75×0.75) + 左右铸造耳 + 顶盖，与炮塔融合
- **指挥塔**：从半径 0.10 大圆柱+半球 → **缩小至 0.085** 扁平圆柱 + 舱门，位置后移偏左
- **车体高度**：`HULL_HI` 从 0.55 **降至 0.48**，更接近实车低矮轮廓
- **翼子板**：加宽加厚 + 前/后挡泥板弯曲
- **负重轮**：r 从 0.14 **加大至 0.155**
- **履带宽度**：加宽至 0.065
- **新增细节**：驾驶员/机枪手双舱盖、装填手舱盖、天线杆、同轴机枪、油箱固定架、圆头斧、排气管保护罩

### v0.17.2 — 版本号修复（2026-05-06）
**修复**：
- 修正 `index.html` 中 3 处硬编码版本号：菜单显示(`menu-version`)、更新日志标题(`cl-title`)、`console.log()` 日志，全部从 `v0.16.1` 更新为当前版本号
- 同步修正 `README.md` 版本号

### v0.17.1 — T-34/85 模型技术图纸对标修正（2026-05-06）
**修正**：基于 T-34/85 技术图纸对比模型差异进行精确修正

### v0.17.0 — T-34/85 坦克模型 v3 技术图纸重构（2026-05-06）
**重构**：基于技术图纸全面重构 T-34/85 模型

### v0.16.1 — T-34-85 模型修正（2026-05-06）
**修正**：
- **车体**：改用 ExtrudeGeometry 挤出通体楔形剖面，首上装甲为一条60°连续斜板从车底前缘直达炮塔下方（不再有垂直盒面）
- **炮塔**：下半部改为较厚垂直圆柱壁（截顶圆锥体）+ 上半部浅穹顶上收；炮塔底径1.06 > 车宽0.94，形成"骑"在车体上的视觉效果
- **防盾**：炮塔正面添加突出矩形防盾块（0.32×0.24×0.12），炮管安装于防盾中心，而非直接插入炮塔壳体
- **主炮**：炮管长度增至约1.15（≈车体长 2/3），管径收细至r=0.033，中后段添加环状抽烟器（r=0.051）
- **主动轮**：半径增至0.18，中心提高至y=0.22（高于负重轮y=0.14），带14齿齿缘，与负重轮明显区分
- **履带**：履带片加厚至0.07×0.035，前后弧形段增加至6段更圆滑

### v0.16.0 — T-34-85 后期型 高精度模型（2026-05-06）
**新增**：
- 新增 `models/t34-85.js`：T-34-85 后期型 (Model 1944/1945) 高精度三维模型
- 楔形车体 + 60°首上倾斜装甲（T-34 系列核心识别特征）
- 铸造半球炮塔（倒扣碗+截顶圆锥组合体，整体连续曲面）
- 85mm 细长主炮（含抽烟装置环状凸起 + 炮口加粗）
- 5对大直径等间距负重轮 + 前方小诱导轮 + 后方带齿大主动轮
- 闭合椭圆履带环路（顶部+底部+前后弧形段）
- 矩形火炮防盾 + 偏后圆柱指挥塔 + 驾驶员舱盖
- 支持绿色涂装 (`t34-85-green`) 和沙漠涂装 (`t34-85-desert`)
- 已加入模型预览下拉菜单，单人/对战模式暂不使用

### v0.15.0 — 模型系统重构 + 模型预览（2026-05-06）
**新增**：
- 模型代码从 index.html 抽取到 `models/` 目录，按类别独立管理
- 模型注册表（`modelRegistry.js`）统一管理所有模型，支持权重随机生成
- **模型预览模式**：主菜单新增"🔍 模型预览"按钮，进入后下拉菜单切换模型浏览
  - 鼠标拖拽左右/上下旋转视角
  - 滚轮缩放
  - ESC 或返回按钮退出
- Three.js 改为本地 `three.min.js` 加载，完全离线运行

**变更**：
- 坦克模型独立为 `models/tank.js`，支持 green/desert 两种涂装
- 树木模型独立为 `models/trees.js`（锥形树/球形树）
- 建筑模型独立为 `models/buildings.js`（平房/别墅/公寓）
- 风车模型独立为 `models/windmill.js`，叶片改用 PlaneGeometry 扁平形状

### v0.14.3 — 修复风车叶片旋转中心（二次修复）（2026-05-06）
**修复**：
- v0.14.2 的修复不完整（旋转后的叶片 position 未随角度变化）
- 改为每个叶片根据 `rotation.z` 角度计算位置：`position.x = cos(angle)*0.275, position.y = sin(angle)*0.275`
- 确保十字形叶片的中心点精确位于 bladeGroup 原点（磨坊轴心），bladeGroup 旋转时叶片围绕轴心转动

### v0.14.2 — 修复风车叶片旋转中心（2026-05-06）
**修复**：
- 修复风车磨坊4片叶片围绕错误原点旋转的问题（原来是围绕 bladeGroup 一侧的偏移点旋转，且0°/180°和90°/270°叶片重叠）
- 改为叶片沿 Y 方向偏移半个长度（0.275），使一端在 bladeGroup 中心，4片向四个方向辐射

### v0.14.1 — 修复爆炸粒子渐隐效果（2026-05-06）
**修复**：
- 修复爆炸粒子停顿卡住后突然消失的问题，粒子现在持续扩散直到完全消失
- 粒子基于生命周期平滑渐隐：`opacity = minLifeRatio^0.5`（火焰）/ `minLifeRatio^0.3`（烟雾）
- 修正菜单界面版本号显示

### v0.14.0 — 优化爆炸效果逻辑（2026-05-06）
**变更**：
- 障碍物被击中或殉爆时不再产生爆炸火焰，只保留碎块散开效果
- 坦克爆炸时的火焰持续时间缩短至 0.25~0.5 秒

### v0.13.9 — 优化烟雾持续时间（2026-05-06）
**变更**：
- 烟雾粒子持续时间从 2-4 秒缩短到 1-1.5 秒

### v0.13.8 — 修复殉爆后障碍物重复移除（2026-05-06）
**修复**：
- 炮弹击中障碍物触发殉爆后，原障碍物被殉爆系统提前移除导致 parent 为 null 报错
- 移除前增加 `obs.parent` 空值检查

### v0.13.7 — 修复殉爆系统栈溢出（2026-05-06）
**修复**：
- `spawnExplosion` 与 `checkChainExplosion` 无限递归导致 Maximum call stack size exceeded
- `spawnExplosion` 新增 `skipChain` 参数，殉爆链内部爆炸不再重复触发殉爆检测

### v0.13.6 — 坦克爆炸效果 & 殉爆系统（2026-05-06）
**新增**：
- 坦克爆炸时触发大型火焰烟雾粒子（120火焰+80烟雾）
- 殉爆系统：爆炸半径3.5米内的树木/建筑物被连锁引爆
- 敌方坦克不受殉爆影响

### v0.13.5 — 新增坦克损毁效果（2026-05-06）
**新增**：
- 血量低于 50 时，坦克燃起火焰并散发烟雾
- 使用 Three.js Points 粒子系统实现（DamageEffects 类）
- 新增 `fireSmokeParticles.js` 模块
- 双人模式下双方玩家各自独立判断

### v0.13.4 — 修复 createTank 未调用 createPlayerTank（2026-05-06）
**修复**：
- `createTank()` 缺少对 `createPlayerTank(p)` 的调用，导致 `tankGroup` 为 `null`，从而 `resetTank()` 和 `placeCamera()` 报错黑屏

### v0.13.3 — 修复单人模式首次进入黑屏（2026-05-06）
**修复**：
- 单人模式首次进入黑屏：重新创建 WebGLRenderer 解决初始化状态异常

### v0.13.2 — 箭头 behind 翻转 + 单人模式首帧渲染（2026-05-06）
**修复**：
- 箭头方向修复：检测目标是否在摄像机后方，反转方向向量 + 钳制箭头位置
- 单人模式 `enterGame()` 中显式调用 `renderer.render(scene, camera)` 产生可见首帧

### v0.13.1 — 箭头方向修复 + tankState 初始化（2026-05-06）
**修复**：
- 箭头角度公式修正为 `-90°`
- `tankState.yaw` 初始值改为 `Math.PI/2`
- `versusGameLoop` 每帧开头添加全屏 viewport/scissor 复位

### v0.13.0 — 脱离服务器依赖（初始版本）
**变更**：
- Three.js 从 ES Module 切换为 UMD CDN `<script>` 标签
- 不再需要 `python -m http.server`，双击 `index.html` 直接运行

---

## 六、T-34-85 模型技术参数（v6.1 — 当前版本）

> 模型代码：`models/t34-85.js` · **v6.1 基于真实 T-34/85 照片重构** · 已实装单人/双人对战模式 · `models/tank.js` 简化模型保留为备选

### 核心几何参数

| 部件 | 关键参数 | 说明 |
|------|---------|------|
| **车体剖面** | 5 点 Shape：前下→首上斜顶→发动机舱斜顶→尾板上缘→倾斜尾板末端 | `ExtrudeGeometry` 挤出，带倾斜尾板 |
| **车体尺寸** | `HULL_W=0.94, HULL_TOP=0.48, HULL_BOT=0.06, FRONT_Z=0.85` | 降低高度(0.55→0.48)更接近实车 |
| **首上倾角** | ~58°从水平面（`GLACIS_DZ=0.52`） | 船形楔形剖面 |
| **炮塔** | `LatheGeometry` 10 点铸造轮廓曲线，36 段旋转体 | "胖蘑菇"形圆润铸造轮廓 |
| **防盾** | 变形 Sphere(scale 1.1×0.75×0.75) + 左右铸造耳 + 顶盖 | 大型包裹式防盾，与炮塔融合 |
| **指挥塔** | 圆柱 r=0.085, h=0.06 + 扁平舱门 | 小型圆柱体，位置后移偏左 |
| **主炮管** | `Cylinder(r=0.033~0.037, len=1.15)`, 中段抽烟器 r=0.051 | 细长 ZIS-S-53 型 85mm 炮 |
| **负重轮×5** | `r=0.155`, y=0.14, z=-0.40,-0.20,0,0.20,0.40 | 加大负重轮 |
| **诱导轮** | `r=0.11`, y=0.12, z=0.73（前下方） | |
| **主动轮** | `r=0.18`, y=0.22, z=-0.68（后上方），14 齿 | |
| **履带宽度** | `TRACK_W = 0.065` | 加宽 |
| **履带结构** | 底部/顶部密集小段(segL=0.095) + TorusGeometry 半圆弧 | 连续包覆式，无断点 |

### 新增细节（v6+）

| 细节 | 描述 |
|------|------|
| 双舱盖 | 驾驶员舱盖(左) + 机枪手舱盖(右)，各带把手 |
| 装填手舱盖 | 炮塔顶面后方椭圆形舱盖 |
| 天线杆 | 车体左前方 L 形天线 |
| 同轴机枪 | 防盾右侧 DT 机枪 |
| 油箱固定架 | 车体外挂油箱的金属固定架 |
| 排气管保护罩 | 发动机舱后方圆管保护罩 |
| 尾板装甲 | 倾斜后装甲板(-35°) |

### 游戏接口兼容性（userData）

模型 `group.userData` 设置了以下字段（游戏代码依赖）：
- `hull` — 车体 Mesh 引用
- `leftWheels` — 左侧轮组 Group 数组（游戏通过 `w.rotation.x` 驱动旋转）
- `rightWheels` — 右侧轮组 Group 数组
- **重要**：修改模型时需保持这些字段，否则单人/对战模式的轮子动画将失效。

### 快速调参指南

修改 `models/t34-85.js` 顶部常量即可调整比例：

```javascript
const HULL_W = 0.94;          // 车体宽度
const HULL_HI = 0.48;         // 车体高度（原 0.55，已降低）
const GLACIS_DZ = 0.52;       // 首上水平跨度（控制倾角 ~58°）
const WL_R = 0.155;           // 负重轮半径（原 0.14，已加大）
const SPROCKET_R = 0.18;      // 主动轮半径
const SPROCKET_Y = 0.22;      // 主动轮高度
const TRACK_W = 0.065;        // 履带宽度
```

### 视觉验证清单

对照真实 T-34/85 (Model 1944) 照片验证：

1. **炮塔形状**：是否为"胖蘑菇"形圆润铸造轮廓（非六角棱柱/半球穹顶）？
2. **防盾**：是否为大型包裹式铸造件（非方块粘贴）？
3. **指挥塔**：是否为小型扁平圆柱（非巨大圆顶）？
4. **车体高度**：是否低矮紧凑（非高盒子）？
5. **尾部轮廓**：是否有倾斜尾板（非垂直方屁股）？
6. **履带**：是否连续无断点包覆所有车轮？
7. **首上装甲**：侧面看是否呈现清晰的 ~58° 斜线？

### 可能的后续任务

- [x] 将 T-34-85 v6.1 模型替换到单人/对战模式中（替换 `models/tank.js`）
- [ ] 实现炮塔独立旋转（当前游戏未实现）
- [ ] 添加履带纹理/细节（履带板、诱导齿等）
- [ ] 增加更多涂装变体（冬季雪地迷彩、1944年工厂涂装等）
- [ ] 优化炮塔 LatheGeometry 轮廓曲线（可进一步微调10个控制点）
- [ ] 添加车体工具箱、备用履带段、缆绳等外部装备细节

---

## 七、关键设计决策与踩坑记录

### 6.1 `visibility:hidden` vs `display:none`
- `display:none` → Canvas 从布局流移除，`width=0, height=0` → WebGL 初始化异常 → 外部浏览器黑屏
- `visibility:hidden` → Canvas 保留布局空间，WebGL 正常初始化 → 切换可见即渲染

### 6.2 坐标系统
- **世界坐标**：XZ 平面，Y 轴向上。坦克面朝 +Z 即 `yaw=π/2`
- **坦克旋转**：`tankGroup.rotation.set(-pitch, π/2 - yaw, 0)`
- **摄像机位置**：`camPos = (tankX - fx*14, 10, tankZ - fz*14)` — 在坦克正后方

### 6.3 箭头角度映射
- CSS 箭头默认朝下（`transform-origin: 50% 0%`）
- 屏幕角度 `atan2(dy, dx)` 与 CSS 旋转的映射：`rotate = angle * 180/π - 90`
- `behind` 标志：当 `project().z > 1` 时，屏幕坐标翻转，需反转方向向量

### 6.4 分屏渲染注意事项
- 每帧必须先 `setScissorTest(false)` 复位全屏
- 两个 viewport 的 `x + width` 须恰好等于 framebuffer 总宽度
- `halfCssW = Math.floor(cssW/2)` 处理奇数宽度

### 6.5 单人/双人模式切换
- `player1` 和 `player2` 是独立对象，单人模式只用 player1
- `enterGame` 需要移除 player2 的所有 3D 对象和 UI 条
- `enterVersusMode` 需要重建 player1 和 player2
- 全局变量 `tankGroup/leftWheels/rightWheels` 是单人模式的快捷引用

### 6.6 殉爆系统防递归
- `spawnExplosion(pos, skipChain)` 参数控制是否触发殉爆链
- 初始爆炸 `skipChain=false` → 正常触发殉爆
- 殉爆链内部 `skipChain=true` → 阻断递归，防止栈溢出

### 6.7 粒子渐隐实现
- 每个粒子维护 `life` 和 `maxLife`
- 整体透明度基于所有粒子中最小生命周期比例计算
- `pow(lifeRatio, 0.5)` 使渐隐在后期加速（火焰更快消失）
- 粒子生命耗尽后保持最后位置渲染，避免突然消失

---

## 八、已知问题（截至 v0.18.3）

| # | 问题 | 备注 |
|---|------|------|
| 1 | GitHub 推送偶尔超时（网络问题） | Gitee 正常，不影响本地开发；已设 30 秒超时自动跳过 |

### 模型待改进项（供后续迭代参考）

| # | 项目 | 优先级 | 说明 |
|---|------|--------|------|
| 1 | 履带纹理细化 | 中 | 当前为纯色 Box/Torus，可加履带板凸起、诱导齿等细节 |
| 2 | 炮塔轮廓微调 | 中 | LatheGeometry 的 10 个控制点可根据更多角度照片精调 |
| 3 | 车体焊缝/铆钉 | 低 | 细节级增强，当前为平滑表面 |
| 4 | 行动装置动画 | 低 | 负重轮/主动轮/履带可加入旋转联动 |

---

## 九、开发者接力指南

### 在单位电脑设置
1. 复制整个 `坦克对战demo/` 文件夹到单位电脑
2. 确认以下文件结构完整（同一文件夹内）：
   - `index.html`、`three.min.js`、`fireSmokeParticles.js`、`README.md`
   - `models/` 目录及其全部 6 个 JS 文件
3. 双击 `index.html` 即可运行（Three.js 已本地化，完全离线）

### Git 仓库信息
- **GitHub**：`git@github.com:hpmax9696/tank_demo.git`（网络不稳定，偶有超时）
- **Gitee**：`git@gitee.com:hpmax9696/tank_demo.git`（推荐使用）
- 每次代码更新后执行 `git add -A && git commit -m "vX.Y.Z: 描述"`，然后推送两边
- GitHub 推送设置 30 秒超时，失败则跳过留待下次

### OneDrive 备份目录
- `C:\Users\hpmax\OneDrive\共享软件\坦克对战demo\`
- 每次代码更新后同步以下文件：`index.html`、`fireSmokeParticles.js`、`README.md`、`models/*`
- OneDrive 目录也用于实际测试运行

### 更新策略 SOP
```
git add -A
git commit -m "vX.Y.Z: 描述"
git push origin master          # Gitee
git push github master          # GitHub (30秒超时，失败跳过)
xcopy index.html + fireSmokeParticles.js + README.md + models/* → OneDrive目录
```

### 调试建议
1. 打开浏览器开发者工具（F12）查看 Console 日志
2. 当前版本 console.log 输出：`🎮 坦克运动demo v0.18.3 | canvas尺寸管理重构`
3. 菜单左下角显示当前版本号和更新日志
4. 修改代码后强制刷新（Ctrl+F5）确保不使用缓存

### 代码规模（截至 v0.18.3）
- `index.html`：约 1972 行（HTML + CSS + JS 游戏引擎）
- `fireSmokeParticles.js`：约 390 行（粒子系统模块）
- `models/t34-85.js`：约 647 行（T-34/85 v6.1 高精度坦克模型）
- 其他模型文件：约 331 行（tank / trees / buildings / windmill / modelRegistry）
- **总计约 3340 行**

---

## 十、版权与许可

坦克 3D 模型和游戏逻辑为原创实现。  
Three.js 使用 MIT 许可证。  
音频通过 Web Audio API 程序化生成，无版权问题。
