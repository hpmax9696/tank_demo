...
> **当前版本：v0.25.1** | 基于 Three.js 的多模块 3D 浏览器游戏
...
2. 当前版本 console.log：`🎮 坦克运动demo v0.25.1 | 俯仰发射角+焦痕贴坡 | PCFShadow+512+36m跟随 | 四阶段探针 | 陡视角+FOV 45°`
3. 页面右上角有调试信息（版本号/FPS/里程/坦克坐标/可见障碍物数量）
4. 修改代码后 `Ctrl+F5` 强制刷新，或关闭标签页重新访问 localhost 确保不使用缓存

### 代码规模（截至 v0.25.1）
- `index.html`：约 3650 行（HTML + CSS + JS 游戏引擎，含陡视角+雾效+草丛系统+遮挡透明化+性能优化）
