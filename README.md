...
> **当前版本：v0.24.0** | 基于 Three.js 的多模块 3D 浏览器游戏
...
**改进**：01a/01b 地图扩展到300×300（空气墙外50m缓冲区），外圈自然下沉-25m融入雾中；删除所有裙边；02a保持200×200仅删裙边；扩展地图障碍物600个(spawnRadius=148)。
...
2. 当前版本 console.log：`🎮 坦克运动demo v0.24.0 | 地形扩展300×300 | 外圈自然下沉 | 01a/01b扩展 02a保持`
3. 页面右上角有调试信息（版本号/FPS/里程/坦克坐标/可见障碍物数量）
4. 修改代码后 `Ctrl+F5` 强制刷新，或关闭标签页重新访问 localhost 确保不使用缓存

### 代码规模（截至 v0.24.0）
- `index.html`：约 3580 行（HTML + CSS + JS 游戏引擎，含地形扩展300×300+Preetham天空+纹理生成+草丛系统）
...
### v0.24.0 — 地形扩展300×300，删除裙边（2026-05-09）
**改进**：01a/01b扩展到300×300，外圈自然下沉；02a仅删裙边。
