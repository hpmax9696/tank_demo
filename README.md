...
> **当前版本：v0.39.0** | 基于 Three.js 的多模块 3D 浏览器游戏 + 地图编辑器
...
2. 当前版本 console.log：`🎮 坦克运动demo v0.39.0 | WASD驾驶+鼠标瞄准+炮塔旋转/炮管俯仰+重力补偿+绿红准星+手柄弹道预测线`
3. 页面右上角有调试信息（版本号/FPS/里程/坦克坐标/可见障碍物数量）
4. 修改代码后 `Ctrl+F5` 强制刷新，或关闭标签页重新访问 localhost 确保不使用缓存

### 代码规模（截至 v0.39.0）
- `index.html`：约 6800 行（主游戏引擎 + WASD驾驶/鼠标瞄准/炮塔炮管/重力补偿）
