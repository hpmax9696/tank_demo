...
> 3D 坦克对战游戏 | Three.js r160 | v0.79.36 | Codex 协作指引
...
31. **校园丧尸实装（v0.79.36）** - 刷怪丧尸补齐 cfg.type/HP条/受击/完整AI/动画；Hit→Stagger 别名；清掉金福园旧测试丧尸
32. **碾压系统修复（v0.79.36）** - 碰撞推离前判定碾压，死亡丧尸不再参与碰撞，尸体不挡车
33. **瞄准设置（v0.79.36）** - 首页新增设置面板，世界指向/车体指向两种炮塔控制，localStorage 持久化
34. **地图切换碰撞清理（v0.79.36）** - 清空 campus 建筑全局碰撞引用，修复 01a/04a 虚空炮弹
35. **模型工厂/人形修（v0.79.36）** - Torus 分段修复眼镜项链可见；眼镜中梁贴脸；项链水平绕颈；Die root 按骨架平滑下沉
36. **渲染/旧丧尸修（v0.79.36）** - 树冠 InstancedMesh 关视锥剔除；换弹防 Q 重复+立即同步；旧版丧尸 Attack 动画映射修复可正常攻击
