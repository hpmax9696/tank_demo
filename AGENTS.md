...
> 3D 坦克对战游戏 | Three.js r160 | v0.79.37 | Codex 协作指引
...
37. **人类士兵类别（v0.79.37）** - 模型工厂新增 4 变体（保安/步枪兵/霰弹枪兵/火箭筒兵），v1-成年男烘焙，活人肤色+直立动画；服装 __cloth__ 联动（藏青/迷彩 Canvas 贴图）；武器挂 r_hand（警棍/AK/泵动霰弹枪/RPG）
38. **帽子/武器修（v0.79.37）** - 半球 center() 补偿修帽子刺穿；弹匣向枪口弯（rotation.x 负）；霰弹枪部件 z 首尾相接修脱节；握把向枪托弯（rotation.x 正）
39. **士兵动画集（v0.79.37）** - deriveSoldierAnims 三套：保安曲臂蓄力→直臂横扫 129°（躯干转体发力）；枪兵双手 low ready + 步枪贴腮瞄准（头右倾）/霰弹腰射 + 枪口焰 3 连发 + 后坐抖动；火箭筒兵斜背露右肩 + 卸筒上肩双手握把（网格搜索） + 双向火焰 + 战斗部射出
40. **武器仰角公式（v0.79.37）** - y=-sin(臂总屈角+武器补偿) 实测映射表；待机补偿 2.0（枪口前下 28°）/据枪 1.65（水平）——旧 0.62 致枪口朝天
41. **左右镜像修正（v0.79.37）** - 第一人称约定（+X=左/-X=右）：待机枪口左偏 wy+0.4、贴腮头右倾 headZ+0.35、RPG 斜背 ry-0.95 战斗部露右肩、肩扛管位右肩 x-0.22
42. **特效（v0.79.37）** - 火焰节点初始 scale 0.001 隐藏+轨道脉冲点亮（emissive×3 过曝亮黄白）；humanoid_factory 扩展关节 scale 复位改恢复初始值（切动画归隐）
43. **验证体系（v0.79.37）** - verify_poses 16（世界系数值）/verify_soldier_anims 20/模型 22/游戏冒烟 0 错误；投影像素级铁证（机匣/头盔同排 15px）；probe_* 实验脚本沉淀（axis/sweep/larm/grip 系列）
