...
- **战利品掉落系统**：`spawnPickup`/`updatePickups`/`cleanupPickups`，医疗箱100%掉落+旋转悬浮+3m拾取
- **丧尸模型**：`models/enemies.js` `makeZombie`，灰白皮肤+肋骨+牛仔裤+抓握手势（约450三角面）
- **专属拾取音效**：`playPickupSound` 上升音阶叮铃
- **满血穿越**：`player1.hp >= maxHp` 时不拾取道具
- **巡逻防卡**：`wpStuckTimer` 3秒未位移跳过当前路径点
- **敌人间碰撞**：`ENEMY_HALF_W*2` 间距推开
