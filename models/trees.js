...
(function() {
    ...
    function createSphericalCrownGeo() {
        ...
        // 1. 核心内部填充 (4个大球)
        cluster.push([0.00,  0.00,  0.00, 0.20]);
        cluster.push([0.00, -0.06,  0.00, 0.17]);
        cluster.push([0.08,  0.02,  0.06, 0.14]);
        cluster.push([-0.06,  0.02, -0.08, 0.14]);
        cluster.push([0.00,  0.08,  0.00, 0.16]);
        ...
        for (const deg of midLow) {
            const rad = deg * Math.PI / 180;
            const r = R * (0.65 + Math.random() * 0.20);
            const x = r * Math.cos(rad);
            const z = r * Math.sin(rad);
        ...
        for (const deg of lowAngles) {
            const rad = deg * Math.PI / 180;
            const r = R * (0.75 + Math.random() * 0.20);
            const x = r * Math.cos(rad);
            const z = r * Math.sin(rad);
        ...
        for (const deg of topAngles) {
            const rad = deg * Math.PI / 180;
            const r = R * (0.25 + Math.random() * 0.20);
            const x = r * Math.cos(rad);
            const z = r * Math.sin(rad);
    ...
    window.TreeModels = {
        ...
        oak: {
            trunkGeo: new THREE.CylinderGeometry(0.06, 0.10, H * 0.55, 16),
    ...
    function makeConical() {
        ...
        trunk.position.y = tm.trunkOffsetY; trunk.castShadow = true; trunk.receiveShadow = true;
        const crown = new THREE.Mesh(tm.crownGeo, tm.crownMat);
        crown.position.y = tm.crownOffsetY; crown.castShadow = true; crown.receiveShadow = true;
