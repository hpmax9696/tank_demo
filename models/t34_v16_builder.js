...
(function () {
    ...
    switch (node.type) {
      ...
      case 'BentBox':
        return buildBentBox(
          ...
          s[1] || 0.3,
          s[2] || s[0],
          s[3] != null ? s[3] : Math.PI / 6,
          seg[0] || 16
        );
      case 'TaperedBox':
        return buildTaperedBox(
          s[0],
          s[1] || 1,
          s[2] || s[0],
  ...
  function getTrackPlateTransform(dist, tp) {
    const cyF = tp.wheelCenterYFront,
      cyR = tp.wheelCenterYRear;
    const rF = tp.wheelRadiusFront,
      rR = tp.wheelRadiusRear;
    const zF = tp.wheelCenterZFront,
      zR = tp.wheelCenterZRear;
    const zR1 = 1.4,
      zR5 = -2.55,
      wheelR = 0.4;
    ...
    const pD = { z: zR1 - wheelR * Math.cos(angleD), y: 0.4 + wheelR * Math.sin(angleD) };
    const angleE = (-75 * Math.PI) / 180;
    const pE = { z: zR5 - wheelR * Math.cos(angleE), y: 0.4 + wheelR * Math.sin(angleE) };
    ...
    d -= lenAB;
    if (d <= lenBC) {
      const a = Math.PI / 2 + (d / lenBC) * (angleC + 2 * Math.PI - Math.PI / 2);
      pos.z = zF - rF * Math.cos(a);
      pos.y = cyF + rF * Math.sin(a);
      rot.x = a - Math.PI / 2;
      return { pos, rot };
    }
    d -= lenBC;
    ...
    d -= lenEF;
    if (d <= lenFA) {
      const a = angleF + (d / lenFA) * (Math.PI / 2 - angleF);
      pos.z = zR - rR * Math.cos(a);
      pos.y = cyR + rR * Math.sin(a);
      rot.x = a - Math.PI / 2;
    }
    return { pos, rot };
  }
  ...
  function buildTrackChain(node, parentGroup, wheelGroups, isDesert) {
    ...
    const pC = { z: zF - rF * Math.cos(angleC), y: cyF + rF * Math.sin(angleC) };
    const angleD = (-105 * Math.PI) / 180;
    const pD = { z: 1.4 - wheelR * Math.cos(angleD), y: 0.4 + wheelR * Math.sin(angleD) };
    const angleE = (-75 * Math.PI) / 180;
    const pE = { z: -2.55 - wheelR * Math.cos(angleE), y: 0.4 + wheelR * Math.sin(angleE) };
    const angleF = (-75 * Math.PI) / 180;
    const pF = { z: zR - rR * Math.cos(angleF), y: cyR + rR * Math.sin(angleF) };
    const lenAB = Math.sqrt((pB.z - pA.z) ** 2 + (pB.y - pA.y) ** 2);
    const lenBC = (angleC + 2 * Math.PI - Math.PI / 2) * rF;
  ...
  function buildFromConfig(node, parentObj, wheelList, isDesert) {
    ...
    if (
      node.name &&
      (node.name.includes('负重轮') || node.name.includes('主动轮') || node.name.includes('诱导轮'))
    ) {
  ...
  function addTankWheelBolts(tankGroup) {
    ...
    tankGroup.traverse(function (obj) {
      ...
      obj.children.forEach(function (c) {
        if (c.userData && c.userData.isBolt) existing.push(c);
      });
      ...
      const isLeft = _wp.x > 0; // 世界 X 判左右，不依赖 name
      // 轮子 rotation.z=π/2 局部+Y→世界-X；左轮(x>0)螺栓朝外(+X)需局部-Y，故与 isLeft 反号
  ...
  window.ModelRegistry.register(
    'tanks',
    'T-34/85 v1.6 绿色',
    (opts) => T34V16Builder.buildAnimatedT34_85({ ...opts, camoColor: 'green' }).group
  );
  window.ModelRegistry.register(
    'tanks',
    'T-34/85 v1.6 黄色',
    (opts) => T34V16Builder.buildAnimatedT34_85({ ...opts, camoColor: 'desert' }).group
  );
})();
